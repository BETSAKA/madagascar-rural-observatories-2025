# utils/calibrate_presentation_consolidated.R
# ─────────────────────────────────────────────────────────────────
# Shiny app for calibrating presentation figure dimensions AND
# font sizes (legend, axes, strip text, bar labels, etc.).
#
# Includes ALL figures found in .qmd files (matching calibrate_figures.R list).
# Adds the "consolidated" profile.
#
# Settings are saved to data/presentation_sizes_consolidated.yml
...
# ── 1. Parse all .qmd files for figure chunks ─────────────────

#' Extract figure-producing code chunks from a .qmd file
parse_figure_chunks_full <- function(qmd_path) {
...
ui <- fluidPage(
  titlePanel("Presentation Figure Calibrator (Consolidated)"),
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      fluidRow(
        ...
        column(
          6,
          actionButton("reset_fonts_btn", "\u21ba Reset fonts", width = "100%")
        )
      ),
      hr(),
    ...
    mainPanel(
      ...
      tabsetPanel(
        ...
        tabPanel("Code", verbatimTextOutput("chunk_code")),
        tabPanel("All figures", DT::dataTableOutput("all_figs_table"))
      )
    )
  )
)
...
server <- function(input, output, session) {
  # --- Inventory (Full list from all QMDs) ---
  all_chunks_list <- parse_all_fig_chunks(PROJECT_ROOT)
  chunk_labels <- names(all_chunks_list)
  chunk_files <- vapply(all_chunks_list, `[[`, character(1), "file")
  chapters <- unique(chunk_files)
  report_sizes <- load_report_sizes()

  message(
    "Found ",
    length(all_chunks_list),
    " figure chunks in ",
    length(chapters),
    " chapters"
  )
  ...
  observeEvent(input$chapter, {
    ...
    target <- if (
      !is.null(isolate(nav_target$figure)) &&
  ...
  observeEvent(input$prev_btn, {
    ...
    if (length(pos) > 0 && pos[1] > 1) {
      ...
      if (chunk_files[new_pos] != input$chapter) {
        ...
        updateSelectInput(session, "chapter", selected = chunk_files[new_pos])
      } else {
        updateSelectInput(session, "figure", selected = chunk_labels[new_pos])
      }
    }
  })

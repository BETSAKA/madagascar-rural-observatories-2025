...
# à partir des données OR pour les années 1995 à 2015 (et 2025)

# Libraries
library(tidyverse)
library(haven)

# Helper: read Stata file and ensure j5 is always character (2025 data has
# numeric j5 while historical data has character j5)
read_dta_j5 <- function(file) {
  df <- haven::read_dta(file)
  if ("j5" %in% names(df)) {
    df <- mutate(df, j5 = as.character(j5))
  }
  df
}
...
process_revppal <- function(year, path = "data/ROS_MDG_microdata/") {
  # 2025: direct total from SAC4 module (non-agricultural wages)
  if (year == 2025) {
    read_dta_j5(paste0(path, year, "/res_sac4.dta")) %>%
...
process_revsec <- function(year, path = "data/ROS_MDG_microdata/") {
  # 2025: direct total from SAC3 module (agricultural wages)
  if (year == 2025) {
    read_dta_j5(paste0(path, year, "/res_sac3.dta")) %>%
    ...
    read_dta_j5(paste0(path, year, "/res_as.dta")) %>%
      mutate(
        as4 = as.numeric(as4), # As character in 2003
...
process_prod_riz_val <- function(year, path = "data/ROS_MDG_microdata/") {
  stopifnot(year %in% c(1995:2015, 2025))

  household_to_obs <- read_dta_j5(paste0(path, year, "/res_deb.dta")) %>%
    transmute(j5, obs = j0)

  df_r <- read_dta_j5(paste0(path, year, "/res_r.dta"))

  df_dc21 <- read_dta_j5(paste0(path, year, "/res_dc21.dta")) %>%
    left_join(household_to_obs, by = "j5")

  if (year %in% c(1995, 2009:2015, 2025)) {
    prod_riz <- df_r %>%
      mutate(prod_riz = r23) %>%
      group_by(j5, year) %>%
    ...
    prod_riz_a <- df_r %>%
      transmute(
        j5,
        year,
        prod_riz_a = rowSums(across(c(r23a1, r23a2)), na.rm = TRUE)
      ) %>%
      group_by(j5, year) %>%
      summarise(
        prod_riz_a = sum(prod_riz_a, na.rm = TRUE),
        .groups = "drop"
      ) %>%
      left_join(household_to_obs, by = "j5")
    ...
    prod_riz_b <- df_r %>%
      transmute(
        j5,
        year,
        prod_riz_b = rowSums(across(c(r23b1, r23b2)), na.rm = TRUE)
      ) %>%
      group_by(j5, year) %>%
      summarise(
        prod_riz_b = sum(prod_riz_b, na.rm = TRUE),
        .groups = "drop"
      ) %>%
      left_join(household_to_obs, by = "j5")
    ...
    stop(
      "Année non prise en charge (pas de variables disponibles dans res_r.dta)"
    )
  }
...
process_charge_riz <- function(year, path = "data/ROS_MDG_microdata/") {
  ...
  # --- 2025 branch: MO from res_mor, no inputs, location from res_r ---
  if (year == 2025) {
    household_to_obs <- read_dta_j5(paste0(path, year, "/res_deb.dta")) %>%
      select(j5, obs = j0) %>%
      distinct()

    # Main d'œuvre riz from res_mor (sum of mor12 = salaire total par opération)
    main_oeuvre <- read_dta_j5(paste0(path, year, "/res_mor.dta")) %>%
      group_by(j5, year) %>%
      summarise(coutmori = sum(mor12, na.rm = TRUE), .groups = "drop")

    # No inputs module in 2025 (res_itb gone)
    coutint <- tibble(j5 = character(), year = integer(), coutint = numeric())

    # Métayage/location from res_r (same logic as historical)
    px_paddy_obs <- read_dta_j5(paste0(path, year, "/res_dc21.dta")) %>%
      left_join(household_to_obs, by = "j5") %>%
      group_by(obs, year) %>%
      summarise(
        dc22 = sum(dc22, na.rm = TRUE),
  ...
  px_paddy_obs <- read_dta_j5(paste0(path, year, "/res_dc21.dta")) %>%
    ...
    mutate(
      pxpaddy_obs = dc25 / dc22, # prix / 1000 en 1997
  ...
  df <- read_dta_j5(paste0(path, year, file_mo))
  ...
  main_oeuvre <- if (year == 1995) {
    ...
    df %>%
      mutate(across(all_of(required[-1]), ~ replace_na(.x, 0))) %>%
      mutate(
        # manque : mo11f mo11g mo11h
        salarie = mo11b *
          mo11d +
          mo11b * mo11d2 + #pour les hommes
          mo11c * mo11e +
          mo11c * mo11e2, # pour les femmes
        tache = mo12 + mo12b,
        entraide = mo23 + mo24,
        coutmori = salarie + tache + entraide
      ) %>%
  ...
  coutint <- tryCatch(
    {
      read_dta_j5(paste0(path, year, "/res_itb.dta")) %>%
        group_by(j5, year) %>%
        summarise(coutint = sum(ita2, na.rm = TRUE), .groups = "drop")
    },
    error = function(e) {
      tibble(j5 = character(), year = year, coutint = NA_real_)
    }
  )
  ...
  coutmetloc <- tryCatch(
    {
      ...
      coutmetloc2 <- df_r %>%
        ...
        group_by(j5, year) %>%
        summarise(
          coutmetloc2 = sum(coutmetloc2, na.rm = TRUE),
          .groups = "drop"
        )
    ...
    error = function(e) {
      tibble(j5 = character(), year = year, coutmetloc = NA_real_)
    }
  )
...
process_prodcu_val <- function(year, path = "data/ROS_MDG_microdata/") {
  # 2025: c2 = production (kg), c4a = % vendu, c6b = montant vente (Ar)
  # prix_cu = c6b / (c2 * c4a / 100) i.e. price per kg from sales data
  if (year == 2025) {
    df <- read_dta_j5(paste0(path, year, "/res_c.dta")) %>%
      mutate(
        obs = substr(j5, 1, 2),
        cult = paste0(c1, c37),
        qte = replace_na(c2, 0)
    ...
    prix_cu <- df %>%
      mutate(qte_vendue = c2 * c4a / 100) %>%
      group_by(cult, obs, year) %>%
      summarise(
        qte_vendue = sum(qte_vendue, na.rm = TRUE),
        c6b = sum(c6b, na.rm = TRUE),
        .groups = "drop"
      ) %>%
      mutate(prix_cu = if_else(qte_vendue > 0, c6b / qte_vendue, NA_real_)) %>%
  ...
  if (year == 1995) {
    ...
    df <- read_dta_j5(paste0(path, year, "/res_c19.dta"))
...
process_coutmocu <- function(year, path = "data/ROS_MDG_microdata/") {
  # 2025: no res_mo3 module — culture labor costs not available separately
  if (year == 2025) {
    # Return empty tibble (will be treated as 0 after merge)
    return(tibble(j5 = character(), year = integer(), coutmocu = numeric()))
  }

  file <- paste0(path, year, "/res_mo3.dta")
  if (!file.exists(file)) {
    stop("Fichier manquant : ", file)
  }

  df <- read_dta_j5(file) %>% mutate(year = year)
...
process_coutintcu <- function(year, path = "data/ROS_MDG_microdata/") {
  if (year == 1995) {
    return(NULL)
  } # pas de données en 1995

  # 2025: no res_itb module — input costs not available
  if (year == 2025) {
    return(tibble(j5 = character(), year = integer(), coutintcu = numeric()))
  }
  ...
  if (!"itb5" %in% names(df)) {
    stop("Variable 'itb5' absente pour l'année ", year)
  }
...
process_coutloccu <- function(year, path = "data/ROS_MDG_microdata/") {
  if (year == 1995) {
    return(NULL)
  }

  # 2025: location costs from res_fr34 (parcels taken in)
  # fr34n = rente payée en argent (1000 Ar), fr34o = rente payée en produit (kg)
  # For non-rice parcels only (fr30c1 != 1); rice location is in charge_riz
  if (year == 2025) {
    fr34 <- read_dta_j5(paste0(path, year, "/res_fr34.dta"))
    ...
    # Product rent (fr34o in kg): needs price conversion via culture prices
    # Use paddy price as proxy for product rents on non-rice parcels
    household_to_obs <- read_dta_j5(paste0(path, year, "/res_deb.dta")) %>%
      select(j5, obs = j0) %>%
      distinct()
  ...
  if (year == 1996) {
    df <- read_dta_j5(paste0(path, year, "/res_c1.dta"))
...
process_rev_cu <- function(year, path = "data/ROS_MDG_microdata/") {
  prod <- process_prodcu_val(year, path)
  coutmo <- process_coutmocu(year, path)
  ...
  if (year == 1995) {
    ...
    coutint <- process_coutintcu(year, path)
    coutloc <- process_coutloccu(year, path)

    # Fonction pour créer un tibble vide avec colonnes données si NULL
    ensure_tbl <- function(df, cols) {
      if (is.null(df)) {
        df <- tibble(
          !!!rlang::set_names(rep(list(NA), length(cols)), names(cols))
        )
    ...
    full <- prod %>%
      ...
      mutate(across(
        c(prodcu_val, coutmocu, coutintcu, coutloccu),
        ~ replace_na(., 0)
      )) %>%
...
process_revel <- function(year, path = "data/ROS_MDG_microdata/") {
  ...
  # --- 2025 branch: simplified (no CIE, no ele_e/ele_l2) ---
  if (year == 2025) {
    ele <- read_dta_j5(paste0(path, year, "/res_ele.dta"))
    pe <- read_dta_j5(paste0(path, year, "/res_pe.dta"))

    # Ventes d'animaux non-bovins: ele_i2 in 1000 Ar
    vente_otrani <- ele %>%
    ...
    revel <- full_join(vente_otrani, vente_nobeef, by = c("j5", "year")) %>%
      mutate(
        vente_otrani = replace_na(vente_otrani, 0),
        vente_nobeef = replace_na(vente_nobeef, 0),
        revel = vente_otrani + vente_nobeef
      ) %>%
  ...
  if (
    !file.exists(ele_path) || !file.exists(pe_path) || !file.exists(cie_path)
  ) {
  ...
  vente_ani <- ele %>%
    ...
    group_by(j5, year) %>%
    summarise(
      across(c(vente_bovin, vente_otrani), ~ sum(.x, na.rm = TRUE)),
      .groups = "drop"
    )
  ...
  if (year == 1995) {
    ...
    vente_lait <- if (length(modalites) >= 1) {
      if_else(pe$pe_lig == modalites[1], pe$valeur, 0)
    } else {
      0
    }
    vente_oeuf <- if (length(modalites) >= 2) {
      if_else(pe$pe_lig == modalites[2], pe$valeur, 0)
    } else {
      0
    }
    vente_beef <- if (length(modalites) >= 3) {
      if_else(pe$pe_lig == modalites[3], pe$valeur, 0)
    } else {
      0
    }
    vente_meat <- if_else(
  ...
  conso_ani_val <- conso_ani %>%
    ...
    group_by(j5, year) %>%
    summarise(
      conso_ani_val = sum(conso_ani_val, na.rm = TRUE),
      .groups = "drop"
    )

  # Charges élevage : consommation intermédiaire
  cie <- read_dta_j5(paste0(path, year, "/res_cie.dta")) %>%
...
process_revpeche <- function(year, path = "data/ROS_MDG_microdata/") {
  ...
  # 2025: fishing module removed
  if (year == 2025) {
    return(tibble(j5 = character(), year = 2025L, revpeche = numeric()))
  }
  ...
  if (
    !file.exists(ppec_path) ||
      !file.exists(cipec_path) ||
      !file.exists(mopec_path)
  ) {
    warning("Fichiers manquants pour l'année ", year, ". Retourne NA.")
    return(tibble(j5 = character(), year = year, revpeche = numeric()))
  }
...
process_rente_riz <- function(year, path = "data/ROS_MDG_microdata/") {
  ...
  # 2025: rent received from rice paddies given out (fr33, fr33c == 1)
  # fr33m1 = cash rent (1000 Ar), fr33m2 = product rent in Ar equivalent
  if (year == 2025) {
    fr33 <- read_dta_j5(paste0(path, year, "/res_fr33.dta"))
    ...
    rente_riz <- fr33 %>%
      ...
      group_by(j5, year) %>%
      summarise(
        rente_riz = sum(rente_argent + rente_produit, na.rm = TRUE),
        .groups = "drop"
      )
  ...
  df_r <- read_dta_j5(paste0(path, year, "/res_r.dta"))
  df_deb <- read_dta_j5(paste0(path, year, "/res_deb.dta")) %>%
    transmute(j5, obs = j0)
  df_deb <- df_deb %>%
...
process_rente_cu <- function(year, path = "data/ROS_MDG_microdata/") {
  ...
  # 2025: rent received from non-rice parcels given out (fr33, fr33c != 1)
  # fr33m1 = cash rent (1000 Ar), fr33m2 = product rent in Ar equivalent
  if (year == 2025) {
    fr33 <- read_dta_j5(paste0(path, year, "/res_fr33.dta"))
    ...
    rente_cu <- fr33 %>%
      filter(fr33c != 1) %>%
      mutate(
        rente_argent = replace_na(fr33m1, 0) * 1000,
        rente_produit = replace_na(fr33m2, 0) * 1000
      ) %>%
      group_by(j5, year) %>%
      summarise(
        rente_cu = sum(rente_argent + rente_produit, na.rm = TRUE),
        .groups = "drop"
      )
  ...
  if (file.exists(file_fr21)) {
    df_fr21 <- read_dta_j5(file_fr21) %>% mutate(year = year)
...
process_autre_rev <- function(year, path = "data/ROS_MDG_microdata/") {
  # 2025: RHA module removed
  if (year == 2025) {
    return(tibble(j5 = character(), year = 2025L, autre_rev = numeric()))
  }

  file <- file.path(path, year, "res_rha.dta")
  if (!file.exists(file)) {
    return(tibble(j5 = character(), year = year, autre_rev = numeric()))
  }
  read_dta_j5(file) %>%
    mutate(
...
process_himo <- function(year, path = "data/ROS_MDG_microdata/") {
  # 2025: HIMO variables (a4b, a4d) removed from res_m_a
  if (year == 2025) {
    return(tibble(j5 = character(), year = 2025L, himo = numeric()))
  }
...
process_vte_par <- function(year, path = "data/ROS_MDG_microdata/") {
  # 2025: parcel sales module (res_fr21) removed
  if (year == 2025) {
    return(tibble(j5 = character(), year = 2025L, vte_par = numeric()))
  }
  ...
  if (year == 2004) {
    ...
    if (!("fr32" %in% names(df) | "fr48" %in% names(df))) {
      warning(
        "Neither fr32 nor fr48 present in 2004 file; returning empty vte_par."
      )
      return(tibble(j5 = character(), year = integer(), vte_par = numeric()))
    }
  ...
  if (file.exists(f21)) {
    ...
    if (!("fr21c" %in% names(df) && "fr21g" %in% names(df))) {
      warning(
        "fr21c/fr21g not found in res_fr21.dta for year ",
        year,
        "; returning empty vte_par."
      )
      return(tibble(j5 = character(), year = integer(), vte_par = numeric()))
    }
    return(
...
process_vente_bovin <- function(year, path = "data/ROS_MDG_microdata/") {
  ...
  if (!file.exists(f)) {
    return(tibble(j5 = character(), year = integer(), vente_bovin = numeric()))
  }
  df <- read_dta_j5(f) %>%
...
process_vente_beef <- function(year, path = "data/ROS_MDG_microdata/") {
  ...
  # 2025: text-based product matching, pe_1i = total sales in Ar
  if (year == 2025) {
    f <- file.path(path, year, "res_pe.dta")
    if (!file.exists(f)) {
      return(tibble(j5 = character(), year = 2025L, vente_beef = numeric()))
    }
    pe <- read_dta_j5(f)
    pe %>%
      mutate(vente_beef = if_else(pe_1 == "Hena", replace_na(pe_1i, 0), 0)) %>%
      group_by(j5, year) %>%
    ...
    pe %>%
      mutate(
        valeur = as.numeric(pe_e) * as.numeric(pe_c),
        vente_beef = if_else(pe_lig == 3, valeur, 0)
      ) %>%
      group_by(j5, year) %>%
      summarise(vente_beef = sum(vente_beef, na.rm = TRUE), .groups = "drop")
...
process_vte_equip <- function(year, path = "data/ROS_MDG_microdata/") {
  # 2025: equipment sales module (res_vb) removed
  if (year == 2025) {
    return(tibble(
      j5 = character(),
      year = 2025L,
  ...
  read_dta_j5(f) %>%
    mutate(year = if (!"year" %in% names(.)) year else year) %>%
    mutate(across(
      c(vb2a, vb2b, vb2c, vb2d, vb2e, vb2f, vb2g),
    ...
    group_by(j5, year) %>%
    summarise(
      across(c(vte_biens, vte_equip), ~ sum(.x, na.rm = TRUE)),
      .groups = "drop"
    )
}
...
process_decap <- function(year, path = "data/ROS_MDG_microdata/") {
  ...
  reduce(list(vp, vbov, vbee, veq), full_join, by = c("j5", "year")) %>%
    mutate(across(
      c(vte_par, vente_bovin, vente_beef, vte_equip),
      ~ coalesce(.x, 0)
    )) %>%
...
compute_income_all_years <- function(

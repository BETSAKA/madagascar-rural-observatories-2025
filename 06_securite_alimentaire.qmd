---
output: html_document
editor_options: 
  chunk_output_type: console
---

# Sécurité alimentaire

L'analyse de la sécurité alimentaire permet de caractériser la vulnérabilité des ménages en examinant la période de soudure, les modes de consommation de base ainsi que les diverses stratégies d'adaptation mobilisées pour faire face au stress alimentaire.

<!-- ::: {.callout-warning} -->
<!-- ## Travail de vérification en cours -->
<!-- Les résultats présentés ci-dessous sont provisoires et pourront être ajustés ou enrichis par les informations supplémentaires recueillies lors des restitutions locales. -->
<!-- ::: -->

```{r data_loading_sa}
options(encoding = "UTF-8")

library(tidyverse)
library(haven)
library(gt)

# Loading observatories data
res_deb <- read_dta("data/ROS_MDG_microdata/2025/res_deb.dta")
# Loading food security data
res_sa <- read_dta("data/ROS_MDG_microdata/2025/res_sa.dta")
# Loading food security strategy data
res_ssa <- read_dta("data/ROS_MDG_microdata/2025/res_ssa.dta")

# Préparer la variable Observatory dans res_deb
res_deb <- res_deb %>%
  mutate(
    Observatory = case_when(
      j0 == 3  ~ "Marovoay",
      j0 == 21 ~ "Alaotra",
      TRUE     ~ as.character(j0)
    ),
    j4bis = case_when(
      str_starts(str_to_lower(j4), "bepako")          ~ "Bepako",
      str_starts(str_to_lower(j4), "ampijoroa")       ~ "Ampijoroa",
      str_starts(str_to_lower(j4), "maroala")          ~ "Maroala",
      str_starts(str_to_lower(j4), "madiromiongana")   ~ "Madiromiongana",
      TRUE ~ j4
    )
  )

source("utils/helpers_report.R")
source("utils/sites.R")
source("utils/downloadable_output.R")

# Add Site column
res_deb <- assign_site(res_deb)
res_deb <- res_deb %>% filter_for_profile()

res_deb_site <- res_deb |> expand_sites_for_profile()

# Pre-joined datasets for site-level downloads
sa_full  <- res_sa  %>% left_join(res_deb %>% select(j5, Observatory, Site, j4bis), by = "j5") %>% filter(!is.na(Observatory))
ssa_full <- res_ssa %>% left_join(res_deb %>% select(j5, Observatory, Site), by = "j5") %>% filter(!is.na(Observatory))
```

```{r stats_sa}
#| include: false
# --- Statistiques clés pour le texte narratif ---
sa_obs <- res_sa %>% left_join(res_deb, by = "j5")
n_men_sa <- n_distinct(sa_obs$j5)

# Soudure
soud_s <- sa_obs %>%
  group_by(Observatory) %>%
  summarise(moy = round(mean(sal2d, na.rm = TRUE), 1),
            med = median(sal2d, na.rm = TRUE), .groups = "drop")
soud_moy_a <- soud_s %>% filter(Observatory == "Alaotra") %>% pull(moy)
soud_moy_m <- soud_s %>% filter(Observatory == "Marovoay") %>% pull(moy)
soud_med_a <- soud_s %>% filter(Observatory == "Alaotra") %>% pull(med)
soud_med_m <- soud_s %>% filter(Observatory == "Marovoay") %>% pull(med)

# Stress alimentaire
stress_s <- sa_obs %>%
  group_by(Observatory) %>%
  summarise(moy = round(mean(sal15a, na.rm = TRUE), 1),
            med = median(sal15a, na.rm = TRUE), .groups = "drop")
stress_moy_a <- stress_s %>% filter(Observatory == "Alaotra") %>% pull(moy)
stress_moy_m <- stress_s %>% filter(Observatory == "Marovoay") %>% pull(moy)

# Pct ménages en soudure > 0 mois
pct_soud_a <- sa_obs %>% filter(Observatory == "Alaotra") %>%
  summarise(pct = round(mean(sal2d > 0, na.rm = TRUE) * 100, 1)) %>% pull(pct)
pct_soud_m <- sa_obs %>% filter(Observatory == "Marovoay") %>%
  summarise(pct = round(mean(sal2d > 0, na.rm = TRUE) * 100, 1)) %>% pull(pct)
```

## Disponibilité alimentaire

La sécurité alimentaire des ménages ruraux est étroitement liée au calendrier agricole et à la gestion des stocks.

::: {.content-hidden when-profile="marovoay"}
::: {.obs-alaotra}
Dans l'Alaotra, `r pct_soud_a` % des ménages déclarent au moins un mois de soudure.
:::
:::

::: {.content-hidden when-profile="alaotra"}
::: {.obs-marovoay}
A Marovoay, `r pct_soud_m` % des ménages déclarent au moins un mois de soudure.
:::
:::

### Période de soudure

La période de soudure désigne les mois durant lesquels les stocks alimentaires du ménage sont insuffisants pour couvrir ses besoins.

::: {.content-hidden when-profile="marovoay"}
::: {.obs-alaotra}
Dans l'Alaotra, la soudure dure en moyenne `r soud_moy_a` mois (médiane : `r soud_med_a`).
:::
:::

::: {.content-hidden when-profile="alaotra"}
::: {.obs-marovoay}
A Marovoay, la soudure dure en moyenne `r soud_moy_m` mois (médiane : `r soud_med_m`).
:::
:::

#### Durée de la soudure par observatoire

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, la durée moyenne de la période de soudure est de 3.8 mois. La moitié des ménages connaissent une période de soudure de 3 mois ou moins . Chaque ménage interrogé subit au moins 1 mois de précarité alimentaire tandis que la durée maximale atteint 12 mois pour certains foyers.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
A Marovoay, l'enquête réalisée auprès de 519 ménages révèle une durée moyenne de soudure de 2.2 mois.  La moitié des ménages connaissent une période de soudure de 2 mois ou moins . Bien que le maximum observé soit de 11 mois, certains ménages ne déclarent aucun mois de soudure .
:::
::::

```{r tbl_soudure_obs}
#| tbl-cap: "Durée de la période de soudure par observatoire"
#| tbl-cap-location: top
#| label: tbl-soudure-obs
# T1 : Statistiques descriptives du nombre de mois de soudure par observatoire
soudure_stats <- res_sa %>%
  left_join(res_deb, by = "j5") %>%
  group_by(Observatory) %>%
  summarise(
    `Nombre de ménages` = n(),
    # Minimum = min(sal2d, na.rm = TRUE),
    Médiane = median(sal2d, na.rm = TRUE),
    Moyenne = round(mean(sal2d, na.rm = TRUE), 1),
    # Maximum = max(sal2d, na.rm = TRUE),
    # `Écart-type` = round(sd(sal2d, na.rm = TRUE), 1),
    .groups = "drop"
  )

soudure_stats %>%
  gt() %>%
  tab_header(
    title = "",
    subtitle = "Nombre de mois de soudure"
  ) %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_column_labels()
  ) %>% 
  style_table()
```

```{r dl_t1_soudure_obs}
#| echo: false
#| results: asis
dl_tbl_variants("t1_soudure_obs", "09", function(d) {
  d %>%
    group_by(Observatory) %>%
    summarise(
      `Nombre de ménages` = n(),
      # Minimum = min(sal2d, na.rm = TRUE),
      Médiane = median(sal2d, na.rm = TRUE),
      Moyenne = round(mean(sal2d, na.rm = TRUE), 1),
      # Maximum = max(sal2d, na.rm = TRUE),
      # `Écart-type` = round(sd(sal2d, na.rm = TRUE), 1),
      .groups = "drop"
    ) %>% rename(Site = Observatory)
}, sa_full)
```

#### Durée de la soudure par hameau et observatoire

La soudure est structurellement plus sévère dans le bassin de l'Alaotra. 


:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, les localités qui se situent au-dessus de la moyenne (3.8 mois) sont Ambohidrony avec 4.8 mois, Ambatomanga avec 4.4 mois, Ambodivoara avec 4.3 mois et Ambatoharanana avec 4.1 mois. À l'inverse, les hameaux présentant une durée de soudure inférieure à la moyenne sont Mangabe avec 3.7 mois, Analamiranga avec 3.6 mois et Feramanga Atsimo avec 3.4 mois, tandis qu'Avaradrano est le plus résiliant en enregistrant la période la plus courte avec 3.1 mois.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
A Marovoay, la durée moyenne de la soudure est plus restreinte et s'établit à 2.2 mois par an. Le hameau de Madiromiongana est le seul de la zone à se situer au-dessus de cette moyenne avec une durée de 2.8 mois. Les autres hameaux affichent des statistiques inférieures ou égales à la moyenne de l'observatoire, avec 2.2 mois pour Maroala et une durée commune de 1.9 mois pour Ampijoroa et Bepako.
:::
::::

```{r t2_soudure_hameau}
#| include: false
# T2 : Nombre moyen de mois de soudure par hameau et observatoire
soudure_hameau <- res_sa %>%
  left_join(res_deb, by = "j5") %>%
  group_by(Observatory, j4bis) %>%
  summarise(
    n_hh = n(),
    mean_soudure = round(mean(sal2d, na.rm = TRUE), 1),
    .groups = "drop"
  )

# Ajouter lignes "Ensemble"
soudure_combined <- soudure_hameau %>%
  group_by(Observatory) %>%
  summarise(
    j4bis = "Ensemble",
    mean_soudure = round(weighted.mean(mean_soudure, n_hh), 1),
    n_hh = sum(n_hh),
    .groups = "drop"
  ) %>%
  bind_rows(soudure_hameau, .) %>%
  arrange(Observatory, j4bis == "Ensemble", j4bis)

soudure_combined %>%
  obs_gt() %>%
  tab_header(title = "Durée moyenne de la soudure par hameau et observatoire") %>%
  cols_label(
    j4bis = "Hameau",
    n_hh = "Nb ménages",
    mean_soudure = "Mois de soudure (moy.)"
  ) %>%
  cols_hide(columns = n_hh) %>%
  tab_style(
    style = list(cell_text(weight = "bold"), cell_fill(color = "#F0F0F0")),
    locations = cells_body(rows = j4bis == "Ensemble")
  ) %>%
  tab_options(row_group.font.weight = "bold") %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_column_labels()
  )
```

```{r dl_t2_soudure_hameau}
#| echo: false
#| results: asis
dl_tbl_variants("t2_soudure_hameau", "09", function(d) {
  d %>%
    group_by(Observatory) %>%
    summarise(
      `Nb ménages` = n(),
      `Mois de soudure (moy.)` = round(mean(sal2d, na.rm = TRUE), 1),
      .groups = "drop"
    ) %>% rename(Site = Observatory)
}, sa_full)
```

```{r fig_soudure_hameau}
#| echo: false
#| label: fig-soudure-hameau
#| fig-cap: "Durée moyenne de la soudure par hameau. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
soudure_combined %>%
  filter(j4bis != "Ensemble") %>%
  ror_bar_grouped(x = j4bis, y = mean_soudure, fill = Observatory,
                  y_label = "Mois de soudure (moy.)",
                  direction = "horizontal", facet = FALSE,
                  show_pct = TRUE, pct_suffix = " mois")
```

```{r dl_fig_soudure_hameau}
#| echo: false
#| results: asis
dl_fig_variants("fig-soudure-hameau", "09", function(d) {
  d %>%
    filter(!is.na(sal2d)) %>%
    group_by(Observatory, j4bis) %>%
    summarise(mean_soudure = round(mean(sal2d, na.rm = TRUE), 1), .groups = "drop") %>%
    rename(Type = j4bis, pct = mean_soudure) %>%
    make_bar_obs(show_pct = TRUE, pct_suffix = " mois")
}, sa_full, width = 10, height = 6)
```

#### Distribution du nombre de mois de soudure

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, la période de soudure est relativement longue pour une part importante des ménages. La distribution montre une concentration des ménages autour de 2 à 5 mois de soudure avec un pic à 2 mois pour 25% des ménages, 21 % en déclarent 3 mois, 16 % en déclarent 4 mois, 14 % en déclarent 5 mois et 11 % connaissent 11 mois de soudure.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
A Marovoay, la distribution met en évidence qu'une large majorité des ménages subissent une période de soudure relativement limitée avec une fréquence maximale de 1 mois puis une diminution progressive lorsque la durée de la soudure augmente. La durée de la soudure est davantage concentrée sur des périodes courtes à modérées : 36 % des ménages déclarent 1 mois de soudure, 27 % en déclarent 2 mois et 17 % en déclarent 3 mois. 
:::
::::

```{r g1_soudure_distrib}
#| include: false
# G1 : Histogramme de la distribution du nombre de mois de soudure
res_sa %>%
  left_join(res_deb, by = "j5") %>%
  expand_sites_for_profile() %>%
  filter(!is.na(sal2d)) %>%
  mutate(sal2d = factor(sal2d)) %>%
  count(Observatory, sal2d) %>%
  mutate(pct = round(n / sum(n) * 100, 1), .by = Observatory) %>%
  ror_bar_v(x = sal2d, y_label = "% ménages", x_angle = 45)
```

```{r fig_soudure_distrib}
#| echo: false
#| label: fig-soudure-distrib
#| fig-cap: "Distribution annuelle de la soudure par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
res_sa %>%
  left_join(res_deb, by = "j5") %>%
  expand_sites_for_profile() %>%
  filter(!is.na(sal2d)) %>%
  mutate(sal2d = factor(sal2d)) %>%
  count(Observatory, sal2d) %>%
  mutate(pct = round(n / sum(n) * 100, 1), .by = Observatory) %>%
  ror_bar_v(x = sal2d, y_label = "% ménages", x_angle = 45
  )
```

```{r dl_g1_soudure_distrib}
#| echo: false
#| results: asis
dl_fig_variants("g1_soudure_distrib", "09", function(d) {
  d %>%
    filter(!is.na(sal2d)) %>%
    mutate(sal2d = factor(sal2d)) %>%
    count(Observatory, sal2d) %>%
    mutate(pct = round(n / sum(n) * 100, 1), .by = Observatory) %>%
    ror_bar_v(x = sal2d, y_label = "% ménages", x_angle = 45)
}, sa_full)
```

#### Calendrier de la soudure

Les deux zones connaissent une période de soudure centrée sur le premier trimestre de l'année civile, correspondant à la période précédant la grande récolte. 

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
À l'Alaotra, la période de soudure est particulièrement intense et prolongée. La période la plus critique se situe entre février et avril pour atteindre un pic de 86.8% en mars. Par contre, bien avant le début de la période des pluies au mois d’octobre, la période de soudure touche déjà 12.8% des ménages au mois d’août et continue à augmenter les mois suivants. Les retombés du mois d’avril jusqu’au mois de juillet coincident avec la période de récolte du riz pluvial et irrigué entre avril et juin (Calendrier agricole, MINAGRI 2001)
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
À Marovoay, la période la plus critique survient entre janvier et mars avec un pic de 65.3% des ménages en février. Ce pic de février est rapidement résorbé le mois suivant avec 52.4% des ménages touchés puis chute à 20.6% en avril. En effet, dès la mi-mars, débutent les récoltes du riz pluvial et de la campagne Asara (Riz irrigué de 2ème saison). De plus, la présence d'autres campagnes notamment le riz intermédiaire Atriatry récolté à partir de mai, ainsi que le riz de contre-saison Jeby en juin et juillet, permet de maintenir la sécurité alimentaire durant une grande partie de la saison sèche (Calendrier agricole, MINAGRI 2001). Cependant, la soudure débute en septembre juste avant le début de la période humide avec 24.9% des ménages concernés . Toutefois, la situation reste relativement stable autour de 22% à 25% jusqu'en décembre avant de bondir à 39.3% en janvier. 
:::
::::

```{r t3_calendrier_soudure}
#| include: false
# T3 : % de ménages en soudure chaque mois, par observatoire
mois_labels <- c(
  "sal2d_10" = "Octobre", "sal2d_11" = "Novembre", "sal2d_12" = "Décembre",
  "sal2d_01" = "Janvier", "sal2d_02" = "Février", "sal2d_03" = "Mars",
  "sal2d_04" = "Avril", "sal2d_05" = "Mai", "sal2d_06" = "Juin",
  "sal2d_07" = "Juillet", "sal2d_08" = "Août", "sal2d_09" = "Septembre"
)

calendrier_soudure <- res_sa %>%
  left_join(res_deb, by = "j5") %>%
  group_by(Observatory) %>%
  summarise(
    across(
      starts_with("sal2d_"),
      ~ round(sum(!is.na(.) & . == 1) / n() * 100, 1),
      .names = "{.col}"
    ),
    .groups = "drop"
  ) %>%
  pivot_longer(-Observatory, names_to = "mois_var", values_to = "pct") %>%
  mutate(
    Mois = factor(mois_labels[mois_var], levels = mois_labels)
  )

calendrier_soudure %>%
  select(Observatory, Mois, pct) %>%
  pivot_wider(names_from = Observatory, values_from = pct) %>%
  gt() %>%
  tab_header(
    title = "Calendrier de la soudure par observatoire",
    subtitle = "% de ménages en période de soudure chaque mois"
  ) %>%
  fmt_number(columns = where(is.numeric), decimals = 1) %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_column_labels()
  )
```

```{r dl_t3_calendrier_soudure}
#| echo: false
#| results: asis
dl_tbl_variants("t3_calendrier_soudure", "09", function(d) {
  d %>%
    group_by(Observatory) %>%
    summarise(
      across(starts_with("sal2d_"),
        ~ round(sum(!is.na(.) & . == 1) / n() * 100, 1)),
      .groups = "drop"
    ) %>%
    pivot_longer(-Observatory, names_to = "mois_var", values_to = "pct") %>%
    mutate(Mois = mois_labels[mois_var]) %>%
    select(Site = Observatory, Mois, pct)
}, sa_full)
```

```{r fig_calendrier_soudure}
#| label: fig-calendrier-soudure
#| fig-cap: "Représentation du calendrier de la soudure par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
res_sa %>%
  left_join(res_deb, by = "j5") %>%
  expand_sites_for_profile() %>%
  group_by(Observatory) %>%
  summarise(
    across(starts_with("sal2d_"),
      ~ round(sum(!is.na(.) & . == 1) / n() * 100, 1)),
    .groups = "drop"
  ) %>%
  pivot_longer(-Observatory, names_to = "mois_var", values_to = "pct") %>%
  mutate(Mois = factor(mois_labels[mois_var], levels = mois_labels)) %>%
  ror_bar_v(x = Mois, y_label = "% ménages en soudure", x_angle = 45)
```

```{r dl_g2_calendrier_soudure}
#| echo: false
#| results: asis
dl_fig_variants("fig-calendrier-soudure", "09", function(d) {
  cal <- d %>%
    group_by(Observatory) %>%
    summarise(
      across(starts_with("sal2d_"),
        ~ round(sum(!is.na(.) & . == 1) / n() * 100, 1)),
      .groups = "drop"
    ) %>%
    pivot_longer(-Observatory, names_to = "mois_var", values_to = "pct") %>%
    mutate(Mois = factor(mois_labels[mois_var], levels = mois_labels))
  cal %>%
    ror_bar_grouped(x = Mois, fill = Observatory,
                    y_label = "% ménages en soudure", direction = "vertical",
                    facet = FALSE, x_angle = 45)
}, sa_full)
```

### Stress alimentaire

Le stress alimentaire reflète les mois durant lesquels le ménage éprouve des difficultés à se nourrir correctement, que ce soit en quantité ou en qualité. Sa durée est généralement supérieure à celle de la soudure stricto sensu.

::: {.content-hidden when-profile="marovoay"}
::: {.obs-alaotra}
Dans l'Alaotra, La durée moyenne du stress alimentaire atteint `r stress_moy_a` mois.
:::
:::

::: {.content-hidden when-profile="alaotra"}
::: {.obs-marovoay}
A Marovoay, La durée moyenne du stress alimentaire atteint `r stress_moy_m` mois.
:::
:::

#### Durée du stress alimentaire par observatoire

Bien que le maximum observé soit également de 12 mois, certains ménages ne déclarent aucun mois de stress alimentaire.

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, la durée moyenne du stress alimentaire est de 2.5 mois. La période de soudure de la moitié des ménages est inférieure ou égale à 2 mois et l’autre moitié a une période de soudure supérieure à 2 mois.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
A Marovoay, l'enquête auprès des ménages révèle une durée moyenne de stress alimentaire de 3.3 mois. La période de soudure de la moitié des ménages est de 3 mois ou moins. 
:::
::::

```{r tbl_stress_obs}
#| tbl-cap: "Durée du stress alimentaire par observatoire"
#| tbl-cap-location: top
#| label: tbl-stress-obs

# T4 : Statistiques du stress alimentaire par observatoire
stress_stats <- res_sa %>%
  left_join(res_deb, by = "j5") %>%
  group_by(Observatory) %>%
  summarise(
    `Nombre de ménages` = n(),
    # Minimum = min(sal15a, na.rm = TRUE),
    Médiane = median(sal15a, na.rm = TRUE),
    Moyenne = round(mean(sal15a, na.rm = TRUE), 1),
    # Maximum = max(sal15a, na.rm = TRUE),
    # `Écart-type` = round(sd(sal15a, na.rm = TRUE), 1),
    .groups = "drop"
  )

stress_stats %>%
  gt() %>%
  tab_header(
    title = "",
    subtitle = "Nombre de mois de stress alimentaire (sal15a)"
  ) %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_column_labels()
  ) %>%
  style_table() 
```

```{r dl_t4_stress_obs}
#| echo: false
#| results: asis
dl_tbl_variants("t4_stress_obs", "09", function(d) {
  d %>%
    group_by(Observatory) %>%
    summarise(
      `Nombre de ménages` = n(),
      # Minimum = min(sal15a, na.rm = TRUE),
      Médiane = median(sal15a, na.rm = TRUE),
      Moyenne = round(mean(sal15a, na.rm = TRUE), 1),
      # Maximum = max(sal15a, na.rm = TRUE),
      # `Écart-type` = round(sd(sal15a, na.rm = TRUE), 1),
      .groups = "drop"
    ) %>% rename(Site = Observatory)
}, sa_full)
```

#### Calendrier du stress alimentaire


:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, le pic massif de stress alimentaire enregistré en mars est de 75%, marquant le point critique de la période de soudure juste avant les récoltes de la grande saison. Le stress diminue drastiquement après la récolte pour atteindre son niveau le plus bas en juin (0.0 %) et juillet (2.0 %), témoignant d'une période de relative abondance suite à la mise en marché du riz. Les difficultés commencent à réapparaître progressivement dès le mois de septembre (17.0 %) et octobre (24.0 %), illustrant l'épuisement rapide des stocks pour une partie des foyers.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
A Marovoay,  le niveau maximal de stress alimentaire est atteint en février à 68 %. Ce plateau de difficultés se prolonge en mars avec encore 59 % des ménages touchés. Avec un stress plus précoce et diffus, Marovoay affiche des taux de stress déjà élevés dès le dernier trimestre de l'année civile, avec 35 % en octobre et environ 31 % en novembre-décembre. Même durant les mois les plus favorables (mai à juillet), le stress alimentaire ne descend jamais en dessous de 10 %, indiquant une insécurité alimentaire résiduelle plus persistante.
:::
::::


```{r t5_calendrier_stress}
#| echo: false
#| include: false
#| warning: false
#| message: false
# T5 : % de ménages en stress alimentaire chaque mois

library(dplyr)
library(tidyr)
library(ggplot2)
library(scales)

mois_labels_stress <- c(
  "sal14a_10" = "Octobre", "sal14a_11" = "Novembre", "sal14a_12" = "Décembre",
  "sal14a_01" = "Janvier", "sal14a_02" = "Février", "sal14a_03" = "Mars",
  "sal14a_04" = "Avril", "sal14a_05" = "Mai", "sal14a_06" = "Juin",
  "sal14a_07" = "Juillet", "sal14a_08" = "Août", "sal14a_09" = "Septembre"
)

calendrier_stress <- res_sa %>%
  left_join(res_deb, by = "j5") %>%
  group_by(Observatory) %>%
  summarise(
    across(
      starts_with("sal14a_"),
      ~ round(sum(!is.na(.) & . == 1) / n() * 100, 1),
      .names = "{.col}"
    ),
    .groups = "drop"
  ) %>%
  pivot_longer(-Observatory, names_to = "mois_var", values_to = "pct") %>%
  mutate(
    # ordre chronologique Oct -> Sep
    Mois = factor(mois_labels_stress[mois_var], levels = unname(mois_labels_stress))
  )

# Figure (pas de tableau)
ggplot(calendrier_stress, aes(x = Mois, y = pct)) +
  geom_col(fill = "#2F78B7", width = 0.9) +
  geom_text(aes(label = ifelse(is.na(pct), "", round(pct, 0))),
            vjust = -0.3, size = 3) +
  facet_wrap(~ Observatory, nrow = 1) +
  scale_y_continuous(
    limits = c(0, max(calendrier_stress$pct, na.rm = TRUE) + 8),
    breaks = pretty_breaks(5)
  ) +
  labs(
    # title = "Calendrier du stress alimentaire",
    # subtitle = "% de ménages en stress alimentaire chaque mois",
    x = NULL,
    y = "% ménages en stress alimentaire"
  ) +
  theme_minimal(base_size = 11) +
  theme(
    panel.grid.major.x = element_line(color = "#e6e6e6"),
    panel.grid.major.y = element_line(color = "#e6e6e6"),
    panel.grid.minor = element_blank(),
    strip.text = element_text(face = "bold"),
    axis.text.x = element_text(angle = 45, hjust = 1),
    plot.title = element_text(face = "bold")
  )
```

```{r dl_t5_calendrier_stress}
#| include: false
dl_tbl_variants("t5_calendrier_stress", "09", function(d) {
  mois_labels_stress <- c(
    "sal14a_10" = "Octobre", "sal14a_11" = "Novembre", "sal14a_12" = "Décembre",
    "sal14a_01" = "Janvier", "sal14a_02" = "Février", "sal14a_03" = "Mars",
    "sal14a_04" = "Avril", "sal14a_05" = "Mai", "sal14a_06" = "Juin",
    "sal14a_07" = "Juillet", "sal14a_08" = "Août", "sal14a_09" = "Septembre"
  )
  d %>%
    group_by(Observatory) %>%
    summarise(
      across(starts_with("sal14a_"),
             ~ round(sum(!is.na(.) & . == 1) / n() * 100, 1)),
      .groups = "drop"
    ) %>%
    pivot_longer(-Observatory, names_to = "mois_var", values_to = "pct") %>%
    mutate(Mois = mois_labels_stress[mois_var]) %>%
    select(Site = Observatory, Mois, pct)
}, sa_full)
```

```{r fig_calendrier_stress}
#| label: fig-calendrier-stress
#| fig-cap: "Représentation du calendrier du stress par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
ggplot(calendrier_stress, aes(x = Mois, y = pct)) +
  geom_col(fill = "#2F78B7", width = 0.9) +
  geom_text(aes(label = ifelse(is.na(pct), "", round(pct, 0))),
            vjust = -0.3, size = 3) +
  facet_wrap(~ Observatory, nrow = 1) +
  scale_y_continuous(
    limits = c(0, max(calendrier_stress$pct, na.rm = TRUE) + 8),
    breaks = pretty_breaks(5)
  ) +
  labs(
    x = NULL,
    y = "% ménages en stress alimentaire"
  ) +
  theme_minimal(base_size = 11) +
  theme(
    panel.grid.major.x = element_line(color = "#e6e6e6"),
    panel.grid.major.y = element_line(color = "#e6e6e6"),
    panel.grid.minor = element_blank(),
    strip.text = element_text(face = "bold"),
    axis.text.x = element_text(angle = 45, hjust = 1),
    plot.title = element_text(face = "bold")
  )
```

#### Comparaison soudure et stress alimentaire

::: {.content-visible when-profile="consolidated"}
Le calendrier révèle deux profils distincts : l'Alaotra subit un choc de soudure extrêmement concentré et violent au premier trimestre, tandis que Marovoay présente un stress plus étalé dans le temps, qui débute plus tôt mais atteint un sommet légèrement moins prononcé.
:::

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
À l'Alaotra, la période de soudure est particulièrement intense et prolongée. Les taux de ménages concernés est systématiquement supérieurs à ceux déclarant un stress alimentaire durant la majeure partie de l'année. La période la plus critique se situe entre février et avril atteignant un pic de 86.8 % de ménages en soudure contre 74.8 % en stress alimentaire au mois de mars. Bien avant le début de la saison humide, la période de soudure s'intensifie dès le mois d'août en touchant 12.8 % des ménages pour atteindre 37.5 % en octobre, tandis que le stress alimentaire se manifeste dès le mois de septembre avec un taux de 17 %.  La baisse brutale observée à partir de mai, où le taux de ménages en soudure chute à 3.2 % et celui du stress alimentaire à 2.6 %, coïncide exactement avec le début des récoltes du riz irrigué et du riz pluvial qui s'étendent de mai à juin selon le calendrier agricole de cette zone.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
À Marovoay, les taux de stress alimentaire sont systématiquement plus élevés que ceux de la soudure tout au long de l'année. La période la plus critique survient entre janvier et mars avec un pic en février touchant 65.3 % des ménages pour la soudure et 67.6 % pour le stress alimentaire. Ce pic de février est rapidement résorbé le mois suivant avec 52.4 % de ménages en soudure en mars, puis chute drastiquement à 20.6 % en avril. Ce soulagement précoce s'explique par le début des récoltes du riz pluvial et de la campagne Asara dès la mi-mars selon le calendrier agricole. La présence d'autres cycles rizicoles comme le riz intermédiaire (Atriatry) récolté à partir de mai, et le riz de contre-saison (Jeby) en juin et juillet permet de maintenir la sécurité alimentaire durant la saison sèche bien que le stress alimentaire persiste pour environ 10 % à 17 % des ménages durant cette période. La période de soudure amorce cependant sa progression dès le mois de septembre avec 11.9 % de ménages affectés juste avant la saison humide, pour s'établir à 24.9 % en octobre. À cette même période, le stress alimentaire concerne déjà 35.1 % des foyers, bien que cette difficulté se manifeste dès le mois d’août avec un taux de 13.3 %. Enfin, entre octobre et décembre, les niveaux de soudure et de stress alimentaire demeurent relativement stables, oscillant respectivement entre 22 % et 25 % d'une part, et entre 30 % et 35 % d'autre part, avant de bondir en janvier pour atteindre 39.3 % contre 47.4 %.
:::
::::

```{r fig_soudure_vs_stress}
#| label: fig-soudure-vs-stress
#| fig-cap: "Comparaison soudure et stress alimentaire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
# G3 : Superposition soudure vs stress alimentaire
# Rebuild from expanded data for site-level facets
sa_joined <- res_sa %>%
  left_join(res_deb, by = "j5") %>%
  expand_sites_for_profile()

comparaison <- bind_rows(
  sa_joined %>%
    group_by(Observatory) %>%
    summarise(across(starts_with("sal2d_"),
      ~ round(sum(!is.na(.) & . == 1) / n() * 100, 1)), .groups = "drop") %>%
    pivot_longer(-Observatory, names_to = "mois_var", values_to = "pct") %>%
    mutate(Mois = factor(mois_labels[mois_var], levels = mois_labels),
           Indicateur = "Soudure"),
  sa_joined %>%
    group_by(Observatory) %>%
    summarise(across(starts_with("sal14a_"),
      ~ round(sum(!is.na(.) & . == 1) / n() * 100, 1)), .groups = "drop") %>%
    pivot_longer(-Observatory, names_to = "mois_var", values_to = "pct") %>%
    mutate(Mois = factor(mois_labels_stress[mois_var], levels = mois_labels_stress),
           Indicateur = "Stress alimentaire")
)

comparaison %>%
  ror_bar_grouped(x = Mois, fill = Indicateur,
                  y_label = "% ménages", direction = "vertical",
                  x_angle = 45)
```

```{r dl_g3_soudure_vs_stress}
#| include: false
dl_fig_variants("fig-soudure-vs-stress", "09", function(d) {
  cal_s <- d %>%
    group_by(Observatory) %>%
    summarise(across(starts_with("sal2d_"),
      ~ round(sum(!is.na(.) & . == 1) / n() * 100, 1)), .groups = "drop") %>%
    pivot_longer(-Observatory, names_to = "mois_var", values_to = "pct") %>%
    mutate(Mois = factor(mois_labels[mois_var], levels = mois_labels),
           Indicateur = "Soudure")
  cal_st <- d %>%
    group_by(Observatory) %>%
    summarise(across(starts_with("sal14a_"),
      ~ round(sum(!is.na(.) & . == 1) / n() * 100, 1)), .groups = "drop") %>%
    pivot_longer(-Observatory, names_to = "mois_var", values_to = "pct") %>%
    mutate(Mois = factor(mois_labels_stress[mois_var], levels = mois_labels_stress),
           Indicateur = "Stress alimentaire")
  comp <- bind_rows(cal_s, cal_st)
  comp %>%
    ror_bar_grouped(x = Mois, fill = Indicateur,
                    y_label = "% ménages", direction = "vertical",
                    x_angle = 45)
}, sa_full)
```



### Alimentation de base

Cette section compare les aliments de base consommés aux trois repas, hors et pendant la période de soudure. Le riz constitue la base alimentaire, mais sa place relative varie selon la saison et la localité. Les substitutions opérées pendant la soudure (manioc, patate douce, maïs) témoignent des stratégies d'adaptation alimentaire des ménages.

```{r alim_labels}
# Définir les labels des aliments
alim_labels <- c(
  "1" = "Riz", "2" = "Maïs", "3" = "Patate douce",
  "4" = "Manioc", "5" = "Pain/beignet/galettes", "6" = "Pomme de terre",
  "7" = "Taro", "8" = "Blé/farine de blé", "9" = "Millet/sorgho",
  "11" = "Autre céréale", "12" = "Igname",
  "14" = "Autre racine/tubercule", "15" = "Pâte alimentaire",
  "23" = "Banane", "24" = "Fruit à pain",
  "25" = "Boisson chaude", "26" = "Lait", "27" = "Fruit sauvage",
  "28" = "Rien", "29" = "Brèdes", "30" = "Légumineuse seule",
  "31" = "Légumineuse+Céréale", "32" = "Céréale+Légumes",
  "33" = "Mélange tubercules", "34" = "Tubercule+Céréale",
  "35" = "Tubercule+Légumes", "36" = "Autres mélanges"
)
```

#### Alimentation hors soudure

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, le riz constitue le socle de l'alimentation des ménages pour les trois repas de la journée hors période de soudure avec 97.8% des ménages consomment du riz au petit-déjeuner, 93.5% au déjeuner et 99.6% au dîner.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
À Marovoay, le riz est l'aliment de base prédominant en dehors de la période de soudure. Il représente l'essentiel du petit-déjeuner à 89.9% des ménages, des déjeuners à 95.2%, des dîners pour 99.6%. 
:::
::::

```{r tbl_alim_hors_soudure}
#| tbl-cap: "Alimentation de base hors soudure par observatoire"
#| tbl-cap-location: top
#| label: tbl-alim-hors-soudure

# T6 : Aliment principal aux 3 repas hors soudure, par observatoire
alim_hors <- res_sa %>%
  left_join(res_deb, by = "j5") %>%
  select(Observatory, sal1a, sal1b, sal1c) %>%
  pivot_longer(
    cols = c(sal1a, sal1b, sal1c),
    names_to = "repas_var",
    values_to = "aliment_code"
  ) %>%
  mutate(
    Repas = case_when(
      repas_var == "sal1a" ~ "Petit-déjeuner",
      repas_var == "sal1b" ~ "Déjeuner",
      repas_var == "sal1c" ~ "Dîner"
    ),
    Aliment = alim_labels[as.character(aliment_code)]
  ) %>%
  filter(!is.na(Aliment)) %>%
  count(Observatory, Repas, Aliment) %>%
  group_by(Observatory, Repas) %>%
  mutate(pct = round(n / sum(n) * 100, 1)) %>%
  ungroup() %>%
  filter(pct >= 2) %>%
  arrange(Observatory, Repas, desc(pct))

alim_hors %>%
  select(Observatory, Repas, Aliment, pct) %>%
  obs_gt() %>%
  tab_header(
    title = "",
    subtitle = "Aliments représentant au moins 2% des réponses"
  ) %>%
  cols_label(Repas = "Repas", Aliment = "Aliment", pct = "%") %>%
  fmt_number(columns = pct, decimals = 1) %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_column_labels()
  ) %>%
  tab_options(row_group.font.weight = "bold") %>% 
  style_table()
```

```{r dl_t6_alim_hors_soudure}
#| include: false
dl_tbl_variants("t6_alim_hors_soudure", "09", function(d) {
  d %>%
    select(Observatory, sal1a, sal1b, sal1c) %>%
    pivot_longer(cols = c(sal1a, sal1b, sal1c), names_to = "repas_var", values_to = "aliment_code") %>%
    mutate(
      Repas = case_when(repas_var == "sal1a" ~ "Petit-déjeuner",
                        repas_var == "sal1b" ~ "Déjeuner",
                        repas_var == "sal1c" ~ "Dîner"),
      Aliment = alim_labels[as.character(aliment_code)]
    ) %>%
    filter(!is.na(Aliment)) %>%
    count(Observatory, Repas, Aliment) %>%
    group_by(Observatory, Repas) %>%
    mutate(pct = round(n / sum(n) * 100, 1)) %>%
    ungroup() %>%
    filter(pct >= 2) %>%
    select(Site = Observatory, Repas, Aliment, pct)
}, sa_full)
```

#### Alimentation pendant la soudure

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, le riz demeure l'aliment prédominant pour 92.0% des ménages au petit déjeuner. Pour le déjeuner, les habitudes se diversifient davantage car le riz ne représente plus que 52.6% des réponses tandis que le manioc et le maïs deviennent des substituts importants avec des taux respectifs de 27.4% et 16.7%. Enfin, le dîner reste la structure la plus stable de l'alimentation avec 98.0% des ménages qui continuent de consommer du riz pendant cette période de soudure.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
A Marovoay, le changement est très marqué durant la soudure au petit déjeuner. La part du riz chute à 55.7% tandis que 25.3% des ménages déclarent ne rien consommer du tout. Concernant le déjeuner, le riz ne constitue plus le repas que pour 60.8% des ménages complété par le maïs à 20.7%, le manioc à 11.5% et la patate douce à 4.6%. Pour le dîner, le riz conserve sa place centrale pour 92.5% des ménages, bien que le manioc (2.6%) et le maïs (2.4%) fassent de légères apparitions dans les statistiques de consommation.
:::
::::

```{r tbl_alim_pendant_soudure}
#| tbl-cap: "Alimentation de base pendant la soudure par observatoire"
#| tbl-cap-location: top
#| label: tbl-alim-pendant-soudure

# T7 : Aliment principal aux 3 repas pendant la soudure
alim_pendant <- res_sa %>%
  left_join(res_deb, by = "j5") %>%
  select(Observatory, sal2a, sal2b, sal2c) %>%
  pivot_longer(
    cols = c(sal2a, sal2b, sal2c),
    names_to = "repas_var",
    values_to = "aliment_code"
  ) %>%
  mutate(
    Repas = case_when(
      repas_var == "sal2a" ~ "Petit-déjeuner",
      repas_var == "sal2b" ~ "Déjeuner",
      repas_var == "sal2c" ~ "Dîner"
    ),
    Aliment = alim_labels[as.character(aliment_code)]
  ) %>%
  filter(!is.na(Aliment)) %>%
  count(Observatory, Repas, Aliment) %>%
  group_by(Observatory, Repas) %>%
  mutate(pct = round(n / sum(n) * 100, 1)) %>%
  ungroup() %>%
  filter(pct >= 2) %>%
  arrange(Observatory, Repas, desc(pct))

alim_pendant %>%
  select(Observatory, Repas, Aliment, pct) %>%
  obs_gt() %>%
  tab_header(
    title = "",
    subtitle = "Aliments représentant au moins 2% des réponses"
  ) %>%
  cols_label(Repas = "Repas", Aliment = "Aliment", pct = "%") %>%
  fmt_number(columns = pct, decimals = 1) %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_column_labels()
  ) %>%
  tab_options(row_group.font.weight = "bold") %>% 
  style_table()
```

```{r dl_t7_alim_pendant_soudure}
#| include: false
dl_tbl_variants("t7_alim_pendant_soudure", "09", function(d) {
  d %>%
    select(Observatory, sal2a, sal2b, sal2c) %>%
    pivot_longer(cols = c(sal2a, sal2b, sal2c), names_to = "repas_var", values_to = "aliment_code") %>%
    mutate(
      Repas = case_when(repas_var == "sal2a" ~ "Petit-déjeuner",
                        repas_var == "sal2b" ~ "Déjeuner",
                        repas_var == "sal2c" ~ "Dîner"),
      Aliment = alim_labels[as.character(aliment_code)]
    ) %>%
    filter(!is.na(Aliment)) %>%
    count(Observatory, Repas, Aliment) %>%
    group_by(Observatory, Repas) %>%
    mutate(pct = round(n / sum(n) * 100, 1)) %>%
    ungroup() %>%
    filter(pct >= 2) %>%
    select(Site = Observatory, Repas, Aliment, pct)
}, sa_full)
```

#### Comparaison de l'alimentation au déjeuner : hors vs pendant soudure

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, le riz est l'aliment de base quasi exclusif consommé par 93.5% des ménages hors soudure et va chuter à 52.6% pendant la soudure. Ce recul est compensé par une diversification vers d'autres produits vivriers. Le manioc devient l'aliment de base au déjeuner pour 27.4% des ménages tandis que le maïs concerne 16.7% des ménages.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
À Marovoay, le riz domine le déjeuner hors soudure avec un taux de 95.2%. Pendant la soudure, sa consommation diminue pour atteindre 60.8%. Le maïs devient alors le principal aliment de substitution avec 20.7% des réponses. Les ménages intègrent également d'autres aliments comme le manioc à hauteur de 11.5% et la patate douce pour 4.6% d'entre eux.
:::
::::


```{r g4_comparaison_dejeuner}
#| include: false
# G4 : Comparaison hors soudure vs pendant soudure au déjeuner (top 5 aliments)
comp_dejeuner <- res_sa %>%
  left_join(res_deb, by = "j5") %>%
  expand_sites_for_profile() %>%
  select(Observatory, sal1b, sal2b) %>%
  pivot_longer(
    cols = c(sal1b, sal2b),
    names_to = "periode",
    values_to = "aliment_code"
  ) %>%
  mutate(
    Période = if_else(periode == "sal1b", "Hors soudure", "Pendant soudure"),
    Aliment = alim_labels[as.character(aliment_code)]
  ) %>%
  filter(!is.na(Aliment)) %>%
  count(Observatory, Période, Aliment) %>%
  group_by(Observatory, Période) %>%
  mutate(pct = round(n / sum(n) * 100, 1)) %>%
  ungroup()

# Garder le top 5 des aliments les plus fréquents
top_aliments <- comp_dejeuner %>%
  group_by(Aliment) %>%
  summarise(total = sum(n), .groups = "drop") %>%
  slice_max(total, n = 5) %>%
  pull(Aliment)

comp_dejeuner %>%
  filter(Aliment %in% top_aliments) %>%
  ror_bar_grouped(x = Aliment, fill = Période,
                  y_label = "% ménages", direction = "horizontal")
```

```{r fig_comparaison_dejeuner}
#| echo: false
#| label: fig-comparaison-dejeuner
#| fig-cap: "Représentation de l'alimentation au déjeuner hors vs pendant soudure. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
comp_dejeuner %>%
  filter(Aliment %in% top_aliments) %>%
  ror_bar_grouped(x = Aliment, fill = Période,
                  y_label = "% ménages", direction = "horizontal")
```

```{r dl_g4_comparaison_dejeuner}
#| include: false
dl_fig_variants("fig-comparaison-dejeuner", "09", function(d) {
  comp <- d %>%
    select(Observatory, sal1b, sal2b) %>%
    pivot_longer(cols = c(sal1b, sal2b), names_to = "periode", values_to = "aliment_code") %>%
    mutate(
      Période = if_else(periode == "sal1b", "Hors soudure", "Pendant soudure"),
      Aliment = alim_labels[as.character(aliment_code)]
    ) %>%
    filter(!is.na(Aliment)) %>%
    count(Observatory, Période, Aliment) %>%
    group_by(Observatory, Période) %>%
    mutate(pct = round(n / sum(n) * 100, 1)) %>%
    ungroup()
  top_a <- comp %>% group_by(Aliment) %>%
    summarise(total = sum(n), .groups = "drop") %>%
    slice_max(total, n = 5) %>% pull(Aliment)
  comp %>%
    filter(Aliment %in% top_a) %>%
    ror_bar_grouped(x = Aliment, fill = Période,
                    y_label = "% ménages", direction = "horizontal")
}, sa_full)
```



#### Changement d'aliment de base au déjeuner entre hors et pendant soudure

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
À l'Alaotra, un peu plus de la moitié des ménages, soit 52.2%, consomme du riz au déjeuner durant la période de soudure. Pour le reste des ménages, l’épuisement des stocks entraîne un recours accru à des aliments de substitution comme le manioc et le maïs, consommés respectivement par 23.4 % et 15.3 % des ménages. Ce recours au manioc est d’autant plus marqué qu’il ne constitue l’alimentation de base habituelle que pour 3.6 % des foyers au cours de l’année.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
À Marovoay, une proportion plus élevée de ménages, soit 60.6 %, mange du riz au déjeuner pendant la période de soudure. Pour compenser cette diminution, les ménages se tournent vers des aliments de substitution comme le maïs, le manioc et la patate douce adoptés respectivement par 18.1 %, 9.7 % et 1.2 % d’entre eux. Cette transition vers le maïs et le manioc est d’autant plus marquée que ces aliments ne constituent l’alimentation de base habituelle au déjeuner que pour 2.2 % et 1.6 % des ménages au cours de l’année. En revanche, hors période de soudure, les ménages ne consomment pas de patate douce.
:::
::::

```{r tbl_transition}
#| tbl-cap: "Changement d'aliment de base au déjeuner"
#| tbl-cap-location: top
#| label: tbl-transition
# T8 : Matrice de transition simplifiée (déjeuner)
transition <- res_sa %>%
  left_join(res_deb, by = "j5") %>%
  mutate(
    Hors_soudure = alim_labels[as.character(sal1b)],
    Pendant_soudure = alim_labels[as.character(sal2b)]
  ) %>%
  filter(!is.na(Hors_soudure) & !is.na(Pendant_soudure)) %>%
  count(Observatory, Hors_soudure, Pendant_soudure) %>%
  group_by(Observatory) %>%
  mutate(pct = round(n / sum(n) * 100, 1)) %>%
  ungroup() %>%
  filter(pct >= 1) %>%
  arrange(Observatory, desc(pct))

transition %>%
  select(Observatory, Hors_soudure, Pendant_soudure, n, pct) %>%
  obs_gt() %>%
  tab_header(
    title = "",
    subtitle = "Transitions hors soudure → pendant soudure (≥ 1%)"
  ) %>%
  cols_label(
    Hors_soudure = "Hors soudure",
    Pendant_soudure = "Pendant soudure",
    pct = "%"
  ) %>%
  cols_hide(n) %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_column_labels()
  ) %>%
  tab_options(row_group.font.weight = "bold") %>% 
  style_table()
```

```{r dl_t8_transition}
#| include: false
dl_tbl_variants("t8_transition", "09", function(d) {
  d %>%
    mutate(
      Hors_soudure = alim_labels[as.character(sal1b)],
      Pendant_soudure = alim_labels[as.character(sal2b)]
    ) %>%
    filter(!is.na(Hors_soudure) & !is.na(Pendant_soudure)) %>%
    count(Observatory, Hors_soudure, Pendant_soudure) %>%
    group_by(Observatory) %>%
    mutate(pct = round(n / sum(n) * 100, 1)) %>%
    ungroup() %>%
    filter(pct >= 1) %>%
    arrange(Observatory, desc(pct)) %>%
    select(Site = Observatory, Hors_soudure, Pendant_soudure, n, pct)
}, sa_full)
```

## Stratégies de sécurité alimentaire

Face à l'insécurité alimentaire, les ménages déploient un éventail de stratégies d'adaptation dont la sévérité varie : réduction des portions, substitution d'aliments, endettement, cueillette sauvage, voire décapitalisation. 

### Fréquence des stratégies d'adaptation

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, la stratégie d'adaptation la plus fréquemment mobilisée par les ménages, en cumulant les fréquences quelquefois et souvent, est la réduction de la ration à chaque repas avec un taux de 59%. Elle est suivie par la réduction ou le renoncement aux ingrédients chers qui concerne 52% des foyers et le remplacement de l'aliment de base pour 41%. Plus d'un quart des ménages (26%) déclarent réduire le nombre de repas quotidien. Viennent ensuite la cueillette d’aliments sauvages et l’emprunt d’aliments (21% chacun) et l’endettement pour l’achat de nourriture (13%).
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
À Marovoay, la réduction de la ration à chaque repas étant pratiquée souvent ou quelquefois par 80% des ménages. La réduction ou le renoncement aux ingrédients chers arrive en deuxième position avec 60% des réponses. Le remplacement de l'aliment de base et la réduction du nombre de repas quotidien sont utilisés à parts égales par 44% des foyers. On observe ensuite le recours à l'endettement alimentaire et la réduction des dépenses non essentielles (17% chacun), la cueillette d'aliments sauvages et l'emprunt de nourriture (15% chacun).  
:::
::::

```{r t9_strategies}
#| include: false
# T9 : Fréquence de chaque stratégie par observatoire

# Labels des stratégies
strat_labels <- c(
  "ssa01" = "S'endetter pour acheter des aliments",
  "ssa02" = "Emprunter des aliments",
  "ssa03" = "Remplacer l'aliment de base",
  "ssa04" = "Réduire la ration à chaque repas",
  "ssa05" = "Réduire/renoncer aux ingrédients chers",
  "ssa07" = "Cueillette d'aliments sauvages",
  "ssa08" = "Réduire les dépenses non essentielles",
  "ssa09" = "Envoyer un membre chez un autre ménage",
  "ssa10" = "Vendre des moyens de production",
  "ssa11" = "Vendre d'autres biens du ménage",
  "ssa12" = "Envoyer un membre travailler ailleurs",
  "ssa14" = "Faire une nouvelle activité",
  "ssa15" = "Réduire le nombre de repas",
  "ssa17" = "Autre stratégie"
)

strat_vars <- names(strat_labels)

freq_labels <- c("0" = "Jamais", "1" = "Rarement", "2" = "Quelquefois", "3" = "Souvent")

# Calculer la distribution pour chaque stratégie
strat_data <- res_ssa %>%
  left_join(res_deb, by = "j5") %>%
  select(Observatory, all_of(strat_vars)) %>%
  pivot_longer(
    cols = all_of(strat_vars),
    names_to = "strategie_var",
    values_to = "frequence"
  ) %>%
  filter(!is.na(frequence)) %>%
  mutate(
    Stratégie = strat_labels[strategie_var],
    Fréquence = freq_labels[as.character(frequence)]
  ) %>%
  count(Observatory, Stratégie, Fréquence) %>%
  group_by(Observatory, Stratégie) %>%
  mutate(pct = round(n / sum(n) * 100, 1)) %>%
  ungroup()

# Tableau : % "Quelquefois" + "Souvent" par stratégie et observatoire
strat_resume <- strat_data %>%
  filter(Fréquence %in% c("Quelquefois", "Souvent")) %>%
  group_by(Observatory, Stratégie) %>%
  summarise(pct_freq = sum(pct), .groups = "drop") %>%
  pivot_wider(names_from = Observatory, values_from = pct_freq, values_fill = 0) %>%
  arrange(desc(rowSums(pick(where(is.numeric)))))

strat_resume %>%
  gt() %>%
  tab_header(
    title = "Stratégies d'adaptation les plus fréquentes",
    subtitle = "% de ménages ayant recours « Quelquefois » ou « Souvent »"
  ) %>%
  safe_cols_label(Stratégie = "Stratégie", Alaotra = "Alaotra (%)", Marovoay = "Marovoay (%)") %>%
  fmt_number(columns = where(is.numeric), decimals = 1) %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = cells_column_labels()
  ) %>%
  tab_style(
    style = cell_fill(color = "#e8f4f8"),
    locations = cells_body(rows = seq(1, nrow(strat_resume), 2))
  )
```


```{r dl_t9_strategies}
#| include: false
dl_tbl_variants("t9_strategies", "09", function(d) {
  strat_labels_local <- c(
    "ssa01" = "S'endetter pour acheter des aliments",
    "ssa02" = "Emprunter des aliments",
    "ssa03" = "Remplacer l'aliment de base",
    "ssa04" = "Réduire la ration à chaque repas",
    "ssa05" = "Réduire/renoncer aux ingrédients chers",
    "ssa07" = "Cueillette d'aliments sauvages",
    "ssa08" = "Réduire les dépenses non essentielles",
    "ssa09" = "Envoyer un membre chez un autre ménage",
    "ssa10" = "Vendre des moyens de production",
    "ssa11" = "Vendre d'autres biens du ménage",
    "ssa12" = "Envoyer un membre travailler ailleurs",
    "ssa14" = "Faire une nouvelle activité",
    "ssa15" = "Réduire le nombre de repas",
    "ssa17" = "Autre stratégie"
  )
  sv <- names(strat_labels_local)
  freq_labels_local <- c("0" = "Jamais", "1" = "Rarement", "2" = "Quelquefois", "3" = "Souvent")
  d %>%
    select(Observatory, all_of(sv)) %>%
    pivot_longer(cols = all_of(sv), names_to = "strategie_var", values_to = "frequence") %>%
    filter(!is.na(frequence)) %>%
    mutate(Stratégie = strat_labels_local[strategie_var],
           Fréquence = freq_labels_local[as.character(frequence)]) %>%
    filter(Fréquence %in% c("Quelquefois", "Souvent")) %>%
    count(Observatory, Stratégie) %>%
    group_by(Observatory) %>%
    mutate(pct = round(n / sum(n) * 100, 1)) %>%
    ungroup() %>%
    select(Site = Observatory, Stratégie, pct)
}, ssa_full)
```

```{r g5_strategies_likert}
#| include: false
# G5 : Barres horizontales empilées — profil Likert des stratégies
strat_plot <- res_ssa %>%
  left_join(res_deb, by = "j5") %>%
  expand_sites_for_profile() %>%
  select(Observatory, all_of(strat_vars)) %>%
  pivot_longer(cols = all_of(strat_vars), names_to = "strategie_var", values_to = "frequence") %>%
  filter(!is.na(frequence)) %>%
  mutate(
    Stratégie = strat_labels[strategie_var],
    Fréquence = factor(freq_labels[as.character(frequence)],
                       levels = c("Souvent", "Quelquefois", "Rarement", "Jamais"))
  ) %>%
  count(Observatory, Stratégie, Fréquence) %>%
  group_by(Observatory, Stratégie) %>%
  mutate(pct = round(n / sum(n) * 100, 1)) %>%
  ungroup()

ggplot(strat_plot, aes(x = pct, y = reorder(Stratégie, pct), fill = Fréquence)) +
  geom_col(position = "stack") +
  facet_wrap(~Observatory) +
  labs(
    # title = "Profil de fréquence des stratégies d'adaptation",
    x = "% de ménages",
    y = NULL,
    fill = "Fréquence"
  ) +
  scale_fill_manual(values = c(
    "Souvent" = "#d62728", "Quelquefois" = "#ff7f0e",
    "Rarement" = "#ffbb78", "Jamais" = "#aec7e8"
  )) +
  theme_ror() +
  theme(
    plot.title = element_text(face = "bold", size = 12),
    axis.text.y = element_text(size = 8)
  )
```

```{r fig_strategies_likert}
#| echo: false
#| label: fig-strategies-likert
#| fig-cap: "Profil de fréquence des stratégies d'adaptation. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
ggplot(strat_plot, aes(x = pct, y = reorder(Stratégie, pct), fill = Fréquence)) +
  geom_col(position = "stack") +
  geom_text(
    aes(label = if_else(pct >= 5, as.character(round(pct, 0)), "")),
    position = position_stack(vjust = 0.5),
    color = "black",
    size = 2.8
   ) +
  facet_wrap(~Observatory) +
  labs(
    x = "% de ménages",
    y = NULL,
    fill = "Fréquence"
  ) +
  scale_fill_manual(values = c(
    "Souvent" = "#d62728", "Quelquefois" = "#ff7f0e",
    "Rarement" = "#ffbb78", "Jamais" = "#aec7e8"
  )) +
  theme_ror() +
  theme(
    plot.title = element_text(face = "bold", size = 12),
    axis.text.y = element_text(size = 8)
  )
```

```{r dl_g5_strategies_likert}
#| include: false
dl_fig_variants("fig-strategies-likert", "09", function(d) {
  strat_labels_local <- c(
    "ssa01" = "S'endetter", "ssa02" = "Emprunter aliments",
    "ssa03" = "Remplacer base", "ssa04" = "Réduire ration",
    "ssa05" = "Réduire ingrédients", "ssa07" = "Cueillette",
    "ssa08" = "Réduire dépenses", "ssa09" = "Envoyer membre",
    "ssa10" = "Vendre production", "ssa11" = "Vendre biens",
    "ssa12" = "Travailler ailleurs", "ssa14" = "Nouvelle activité",
    "ssa15" = "Réduire repas", "ssa17" = "Autre"
  )
  sv <- names(strat_labels_local)
  freq_labels_local <- c("0" = "Jamais", "1" = "Rarement",
                          "2" = "Quelquefois", "3" = "Souvent")
  sd <- d %>%
    select(Observatory, all_of(sv)) %>%
    pivot_longer(cols = all_of(sv), names_to = "var", values_to = "freq") %>%
    filter(!is.na(freq)) %>%
    mutate(
      Stratégie = strat_labels_local[var],
      Fréquence = factor(freq_labels_local[as.character(freq)],
                         levels = c("Souvent", "Quelquefois", "Rarement", "Jamais"))
    ) %>%
    count(Observatory, Stratégie, Fréquence) %>%
    group_by(Observatory, Stratégie) %>%
    mutate(pct = round(n / sum(n) * 100, 1)) %>%
    ungroup()
  ggplot(sd, aes(x = pct, y = reorder(Stratégie, pct), fill = Fréquence)) +
    geom_col(position = "stack") +
    geom_text(
      aes(label = if_else(pct >= 5, as.character(round(pct, 0)), "")),
      position = position_stack(vjust = 0.5),
      color = "black",
      size = 2.8
    ) +
    facet_wrap(~Observatory) +
    labs(x = "% de ménages", y = NULL, fill = "Fréquence") +
    scale_fill_manual(values = c(
      "Souvent" = "#d62728", "Quelquefois" = "#ff7f0e",
      "Rarement" = "#ffbb78", "Jamais" = "#aec7e8"
    )) +
    theme_ror() +
    theme(axis.text.y = element_text(size = 8))
}, ssa_full)
```


:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, les principales stratégies d'adaptation adoptées par les ménages face aux difficultés alimentaires sont dominées par la réduction de la ration à chaque repas, citée par 21 % des ménages, suivie de la réduction ou du renoncement aux ingrédients coûteux (19 %) et du remplacement de l'aliment de base (15 %). La réduction du nombre de repas quotidiens concerne également une proportion importante des ménages (10 %). Ces résultats montrent que les ménages privilégient avant tout des ajustements portant sur la qualité et la quantité des repas afin de faire face aux contraintes alimentaires, tandis que la cueillette d'aliments sauvages reste une stratégie relativement peu fréquente (8 %).
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
A Marovoay, la réduction de la ration à chaque repas constitue la principale stratégie d'adaptation, adoptée par 26 % des ménages. Elle est suivie par la réduction ou le renoncement aux ingrédients coûteux (19 %). Le remplacement de l'aliment de base et la réduction du nombre de repas sont chacun mentionnés par 14 % des ménages, traduisant un recours important à des stratégies visant à diminuer les dépenses alimentaires et la consommation. En revanche, la cueillette d'aliments sauvages demeure marginale (5 %). 
:::
::::

```{r g6_top_strategies_data}
# G6 : Top 5 des stratégies les plus fréquentes
top5 <- strat_resume %>%
  slice_head(n = 5) %>%
  pivot_longer(cols = where(is.numeric), names_to = "Observatory", values_to = "pct")
```

```{r fig_top_strategies}
#| label: fig-top-strategies
#| fig-cap: "Stratégies de sécurité alimentaire adoptées par les ménages.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-height: !expr fig_h("fig-top-strategies")
res_ssa %>%
  left_join(res_deb, by = "j5") %>%
  expand_sites_for_profile() %>%
  select(Observatory, all_of(strat_vars)) %>%
  pivot_longer(cols = all_of(strat_vars), names_to = "var", values_to = "freq") %>%
  filter(!is.na(freq)) %>%
  mutate(Stratégie = strat_labels[var],
         Fréquence = freq_labels[as.character(freq)]) %>%
  filter(Fréquence %in% c("Quelquefois", "Souvent")) %>%
  count(Observatory, Stratégie) %>%
  group_by(Observatory) %>%
  mutate(pct = round(n / sum(n) * 100, 1)) %>%
  ungroup() %>%
  {
    top5_strat <- (.) %>%
      group_by(Stratégie) %>%
      summarise(total = sum(pct), .groups = "drop") %>%
      slice_max(total, n = 5) %>%
      pull(Stratégie)
    (.) %>% filter(Stratégie %in% top5_strat)
  } %>%
  make_bar_obs(x = Stratégie, y_label = "% ménages")
```

```{r dl_g6_top_strategies}
#| include: false
dl_fig_variants("fig-top-strategies", "09", function(d) {
  strat_labels_local <- c(
    "ssa01" = "S'endetter", "ssa02" = "Emprunter aliments",
    "ssa03" = "Remplacer base", "ssa04" = "Réduire ration",
    "ssa05" = "Réduire ingrédients", "ssa07" = "Cueillette",
    "ssa08" = "Réduire dépenses", "ssa09" = "Envoyer membre",
    "ssa10" = "Vendre production", "ssa11" = "Vendre biens",
    "ssa12" = "Travailler ailleurs", "ssa14" = "Nouvelle activité",
    "ssa15" = "Réduire repas", "ssa17" = "Autre"
  )
  sv <- names(strat_labels_local)
  freq_labels_local <- c("0" = "Jamais", "1" = "Rarement",
                          "2" = "Quelquefois", "3" = "Souvent")
  sr <- d %>%
    select(Observatory, all_of(sv)) %>%
    pivot_longer(cols = all_of(sv), names_to = "var", values_to = "freq") %>%
    filter(!is.na(freq)) %>%
    mutate(Stratégie = strat_labels_local[var],
           Fréquence = freq_labels_local[as.character(freq)]) %>%
    filter(Fréquence %in% c("Quelquefois", "Souvent")) %>%
    count(Observatory, Stratégie) %>%
    group_by(Observatory) %>%
    mutate(pct = round(n / sum(n) * 100, 1)) %>%
    ungroup()
  top5 <- sr %>%
    group_by(Stratégie) %>%
    summarise(total = sum(pct), .groups = "drop") %>%
    slice_max(total, n = 5) %>% pull(Stratégie)
  sr %>%
    filter(Stratégie %in% top5) %>%
    ror_bar_grouped(x = Stratégie, fill = Observatory,
                    y_label = "% ménages", direction = "horizontal",
                    facet = FALSE)
}, ssa_full)
```

### Sources d'endettement et d'emprunt alimentaire

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, concernant les sources d'endettement pour l'achat alimentaire, les ménages de l'Alaotra se tournent principalement vers la famille à hauteur de 64.1%, suivie des voisins pour 40.6%. Pour ce qui est de l'emprunt d'aliments de base, la famille reste la source la plus sollicitée avec 45.5% suivie des voisins avec 35.0% et enfin des commerçants avec 28.0%.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
À Marovoay, l'endettement pour l'achat de nourriture est contracté en priorité auprès de la famille pour 44.0% des ménages concernés, puis auprès des voisins pour 33.0%, des usuriers pour 17.8%. Quant à l'emprunt d'aliments, il s'effectue majoritairement auprès de la famille à raison de 58.6% tandis que les voisins interviennent pour 27.1%.
:::
::::

```{r t10_sources_endettement}
#| include: false
# T10 : Sources d'endettement alimentaire par observatoire
source_labels <- c(
  "ssa01_01" = "Famille", "ssa01_02" = "Voisins",
  "ssa01_03" = "Usuriers", "ssa01_04" = "Commerçants",
  "ssa01_05" = "Autres"
)

sources_dette <- res_ssa %>%
  left_join(res_deb, by = "j5") %>%
  filter(ssa01 > 0) %>%  # Uniquement les ménages qui s'endettent
  select(Observatory, all_of(names(source_labels))) %>%
  pivot_longer(
    cols = all_of(names(source_labels)),
    names_to = "source_var",
    values_to = "reponse"
  ) %>%
  filter(!is.na(reponse)) %>%
  mutate(
    Source = source_labels[source_var],
    Oui = if_else(reponse == 1, 1L, 0L)
  ) %>%
  group_by(Observatory, Source) %>%
  summarise(
    n_oui = sum(Oui),
    n_total = n(),
    pct = round(n_oui / n_total * 100, 1),
    .groups = "drop"
  )

sources_dette %>%
  select(Observatory, Source, pct) %>%
  pivot_wider(
    names_from = Observatory,
    values_from = pct,
    names_glue = "{Observatory}_pct"
  ) %>%
  gt() %>%
  tab_header(
    title = "Sources d'endettement pour achat alimentaire",
    subtitle = "Parmi les ménages qui s'endettent (ssa01 > 0)"
  ) %>%
  safe_tab_spanner(label = "Alaotra", columns = starts_with("Alaotra")) %>%
  safe_tab_spanner(label = "Marovoay", columns = starts_with("Marovoay")) %>%
  safe_cols_label(
    Source = "Source",
    Alaotra_pct = "%",
    Marovoay_pct = "%"
  ) %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = list(cells_column_labels(), cells_column_spanners())
  )
```

```{r dl_t10_sources_endettement}
#| include: false
dl_tbl_variants("t10_sources_endettement", "09", function(d) {
  src_labs <- c(
    "ssa01_01" = "Famille", "ssa01_02" = "Voisins",
    "ssa01_03" = "Usuriers", "ssa01_04" = "Commerçants",
    "ssa01_05" = "Autres"
  )
  d %>%
    filter(ssa01 > 0) %>%
    select(Observatory, all_of(names(src_labs))) %>%
    pivot_longer(cols = all_of(names(src_labs)),
                 names_to = "source_var", values_to = "reponse") %>%
    filter(!is.na(reponse)) %>%
    mutate(Source = src_labs[source_var],
           Oui = if_else(reponse == 1, 1L, 0L)) %>%
    group_by(Observatory, Source) %>%
    summarise(n_oui = sum(Oui), n_total = n(),
              pct = round(n_oui / n_total * 100, 1), .groups = "drop") %>%
    select(Site = Observatory, Source, n_oui, pct)
}, ssa_full)
```

```{r fig_sources_endettement}
#| echo: false
#| label: fig-sources-endettement
#| fig-cap: "Sources d'endettement alimentaire.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
res_ssa %>%
  left_join(res_deb, by = "j5") %>%
  expand_sites_for_profile() %>%
  filter(ssa01 > 0) %>%
  select(Observatory, all_of(names(source_labels))) %>%
  pivot_longer(cols = all_of(names(source_labels)),
               names_to = "source_var", values_to = "reponse") %>%
  filter(!is.na(reponse)) %>%
  mutate(Type = source_labels[source_var],
         Oui = if_else(reponse == 1, 1L, 0L)) %>%
  group_by(Observatory, Type) %>%
  summarise(pct = round(sum(Oui) / n() * 100, 1), .groups = "drop") %>%
  make_bar_obs(show_pct = TRUE)
```

```{r dl_fig_sources_endettement}
#| echo: false
#| results: asis
dl_fig_variants("fig-sources-endettement", "09", function(d) {
  src_labs <- c(
    "ssa01_01" = "Famille", "ssa01_02" = "Voisins",
    "ssa01_03" = "Usuriers", "ssa01_04" = "Commer\u00e7ants",
    "ssa01_05" = "Autres"
  )
  d %>%
    filter(ssa01 > 0) %>%
    select(Observatory, all_of(names(src_labs))) %>%
    pivot_longer(-Observatory, names_to = "source_var", values_to = "reponse") %>%
    filter(!is.na(reponse)) %>%
    mutate(Source = src_labs[source_var], Oui = if_else(reponse == 1, 1L, 0L)) %>%
    group_by(Observatory, Source) %>%
    summarise(pct = round(sum(Oui) / n() * 100, 1), .groups = "drop") %>%
    rename(Type = Source) %>%
    make_bar_obs(show_pct = TRUE)
}, ssa_full, width = 10, height = 6)
```

```{r t11_sources_emprunt}
#| include: false
# T11 : Sources d'emprunt d'aliments par observatoire
source_emprunt_labels <- c(
  "ssa02_01" = "Famille", "ssa02_02" = "Voisins",
  "ssa02_03" = "Usuriers", "ssa02_04" = "Commerçants",
  "ssa02_05" = "Autres"
)

sources_emprunt <- res_ssa %>%
  left_join(res_deb, by = "j5") %>%
  filter(ssa02 > 0) %>%
  select(Observatory, all_of(names(source_emprunt_labels))) %>%
  pivot_longer(
    cols = all_of(names(source_emprunt_labels)),
    names_to = "source_var",
    values_to = "reponse"
  ) %>%
  filter(!is.na(reponse)) %>%
  mutate(
    Source = source_emprunt_labels[source_var],
    Oui = if_else(reponse == 1, 1L, 0L)
  ) %>%
  group_by(Observatory, Source) %>%
  summarise(
    n_oui = sum(Oui),
    n_total = n(),
    pct = round(n_oui / n_total * 100, 1),
    .groups = "drop"
  )

sources_emprunt %>%
  select(Observatory, Source, pct) %>%
  pivot_wider(
    names_from = Observatory,
    values_from = pct,
    names_glue = "{Observatory}_pct"
  ) %>%
  gt() %>%
  tab_header(
    title = "Sources d'emprunt d'aliments de base",
    subtitle = "Parmi les ménages qui empruntent des aliments (ssa02 > 0)"
  ) %>%
  safe_tab_spanner(label = "Alaotra", columns = starts_with("Alaotra")) %>%
  safe_tab_spanner(label = "Marovoay", columns = starts_with("Marovoay")) %>%
  safe_cols_label(
    Source = "Source",
    Alaotra_pct = "%",
    Marovoay_pct = "%"
  ) %>%
  tab_style(
    style = cell_text(weight = "bold"),
    locations = list(cells_column_labels(), cells_column_spanners())
  )
```

```{r dl_t11_sources_emprunt}
#| include: false
dl_tbl_variants("t11_sources_emprunt", "09", function(d) {
  src_labs <- c(
    "ssa02_01" = "Famille", "ssa02_02" = "Voisins",
    "ssa02_03" = "Usuriers", "ssa02_04" = "Commerçants",
    "ssa02_05" = "Autres"
  )
  d %>%
    filter(ssa02 > 0) %>%
    select(Observatory, all_of(names(src_labs))) %>%
    pivot_longer(cols = all_of(names(src_labs)),
                 names_to = "source_var", values_to = "reponse") %>%
    filter(!is.na(reponse)) %>%
    mutate(Source = src_labs[source_var],
           Oui = if_else(reponse == 1, 1L, 0L)) %>%
    group_by(Observatory, Source) %>%
    summarise(n_oui = sum(Oui), n_total = n(),
              pct = round(n_oui / n_total * 100, 1), .groups = "drop") %>%
    select(Site = Observatory, Source, n_oui, pct)
}, ssa_full)
```

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, les principales sources d’emprunt d’aliments de base proviennent de la famille, citée par 45.5 % des ménages. Les voisins occupent également une place importante avec 35 %, tandis que les commerçants représentent 28 %. Les autres sources restent très peu mobilisées, avec seulement 1.7 % pour les usuriers et 1.7 % pour les autres formes d’emprunt.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
A Marovoay, la famille constitue de loin la principale source d’emprunt d’aliments de base avec 58.6 % des ménages concernés. Les voisins arrivent ensuite avec 27.1 %. Les commerçants occupent une place plus limitée avec 6.9 %, tandis que les usuriers et les autres sources demeurent marginales avec respectivement 5.6 % et 1.4 %.
:::
::::

```{r fig_sources_emprunt}
#| echo: false
#| label: fig-sources-emprunt
#| fig-cap: "Sources d'emprunt d'aliments de base.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
res_ssa %>%
  left_join(res_deb, by = "j5") %>%
  expand_sites_for_profile() %>%
  filter(ssa02 > 0) %>%
  select(Observatory, all_of(names(source_emprunt_labels))) %>%
  pivot_longer(cols = all_of(names(source_emprunt_labels)),
               names_to = "source_var", values_to = "reponse") %>%
  filter(!is.na(reponse)) %>%
  mutate(Type = source_emprunt_labels[source_var],
         Oui = if_else(reponse == 1, 1L, 0L)) %>%
  group_by(Observatory, Type) %>%
  summarise(pct = round(sum(Oui) / n() * 100, 1), .groups = "drop") %>%
  make_bar_obs(show_pct = TRUE)
```

```{r dl_fig_sources_emprunt}
#| echo: false
#| results: asis
dl_fig_variants("fig-sources-emprunt", "09", function(d) {
  src_labs <- c(
    "ssa02_01" = "Famille", "ssa02_02" = "Voisins",
    "ssa02_03" = "Usuriers", "ssa02_04" = "Commer\u00e7ants",
    "ssa02_05" = "Autres"
  )
  d %>%
    filter(ssa02 > 0) %>%
    select(Observatory, all_of(names(src_labs))) %>%
    pivot_longer(-Observatory, names_to = "source_var", values_to = "reponse") %>%
    filter(!is.na(reponse)) %>%
    mutate(Source = src_labs[source_var], Oui = if_else(reponse == 1, 1L, 0L)) %>%
    group_by(Observatory, Source) %>%
    summarise(pct = round(sum(Oui) / n() * 100, 1), .groups = "drop") %>%
    rename(Type = Source) %>%
    make_bar_obs(show_pct = TRUE)
}, ssa_full, width = 10, height = 6)
```


## Indice synthétique de stratégies d'adaptation (CSI simplifié)

Un score simplifié est calculé en sommant les fréquences de toutes les stratégies (0 à 3) pour chaque ménage. Plus le score est élevé, plus le ménage recourt intensément à des stratégies d'adaptation. La présence de quelques valeurs extrêmes supérieures à 25 révèle des situations de forte vulnérabilité alimentaire chez certains ménages. 

<!-- :::: {.content-hidden when-profile="marovoay"} -->
<!-- ::: obs-alaotra -->
<!-- À Alaotra, le score CSI moyen est de 8.4 traduisant un recours relativement important aux stratégies d’adaptation alimentaires. Feramanga Atsimo présente le score le plus élevé (10.1), suivi d’Ambohidrony (9.0). Ambatomanga (8.6), Analamiranga et Mangabe (8.5) affichent des niveaux proches de la moyenne générale, tandis qu’Avaradrano enregistre le score le plus faible (6.8). La boîte à moustaches montre que la majorité des ménages ont des scores concentrés autour de la médiane située près de 8.  -->
<!-- ::: -->
<!-- :::: -->

<!-- :::: {.content-hidden when-profile="alaotra"} -->
<!-- ::: obs-marovoay -->
<!-- À Marovoay, le score CSI moyen est de 9.5 indiquant un recours plus important aux stratégies d’adaptation. Maroala présente le score le plus élevé (10.0), suivi d’Ampijoroa (9.6), de Madiromiongana (9.3) et de Bepako (9.1). La boîte à moustaches montre une médiane autour de 9 et une concentration de la majorité des ménages entre des scores d’environ 5 et 13. -->
<!-- ::: -->
<!-- :::: -->

```{r t12_csi}
# # T12 : Score CSI simplifié par observatoire et hameau
# csi <- res_ssa %>%
#   left_join(res_deb, by = "j5") %>%
#   mutate(
#     csi_score = rowSums(across(all_of(strat_vars), ~replace_na(., 0)))
#   )
# 
# csi_hameau <- csi %>%
#   group_by(Observatory, j4bis) %>%
#   summarise(
#     n_hh = n(),
#     mean_csi = round(mean(csi_score, na.rm = TRUE), 1),
#     .groups = "drop"
#   )
# 
# # Ajouter lignes "Ensemble"
# csi_combined <- csi_hameau %>%
#   group_by(Observatory) %>%
#   summarise(
#     j4bis = "Ensemble",
#     mean_csi = round(weighted.mean(mean_csi, n_hh), 1),
#     n_hh = sum(n_hh),
#     .groups = "drop"
#   ) %>%
#   bind_rows(csi_hameau, .) %>%
#   arrange(Observatory, j4bis == "Ensemble", j4bis)
# 
# csi_combined %>%
#   obs_gt() %>%
#   tab_header(
#     title = "Score CSI simplifié par hameau et observatoire",
#     subtitle = "Somme des fréquences des stratégies (0–3 par stratégie)"
#   ) %>%
#   cols_label(j4bis = "Hameau", n_hh = "Nb ménages", mean_csi = "Score CSI moyen") %>%
#   tab_style(
#     style = list(cell_text(weight = "bold"), cell_fill(color = "#F0F0F0")),
#     locations = cells_body(rows = j4bis == "Ensemble")
#   ) %>%
#   tab_options(row_group.font.weight = "bold") %>%
#   tab_style(
#     style = cell_text(weight = "bold"),
#     locations = cells_column_labels()
#   )
```

```{r dl_t12_csi}
# #| include: false
# dl_tbl_variants("t12_csi", "09", function(d) {
#   sv <- c("ssa01", "ssa02", "ssa03", "ssa04", "ssa05", "ssa07",
#           "ssa08", "ssa09", "ssa10", "ssa11", "ssa12", "ssa14",
#           "ssa15", "ssa17")
#   d %>%
#     mutate(csi_score = rowSums(across(all_of(sv), ~replace_na(., 0)))) %>%
#     summarise(
#       n_hh = n(),
#       mean_csi = round(mean(csi_score, na.rm = TRUE), 1),
#       .by = Observatory
#     ) %>%
#     select(Site = Observatory, n_hh, mean_csi)
# }, ssa_full)
```

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, la distribution du score CSI simplifié présente une médiane autour de 8, avec une dispersion relativement importante des valeurs. La moitié des ménages se situe approximativement entre 4 et 12, ce qui traduit des situations assez variées en matière de stratégies d’adaptation alimentaire. Quelques ménages affichent toutefois des scores particulièrement élevés, dépassant 20 et atteignant environ 25 à 27, ce qui indique des niveaux de vulnérabilité plus marqués pour une partie réduite de la population.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
A Marovoay, la médiane du score CSI simplifié est légèrement plus élevée, autour de 9. Les valeurs sont également dispersées, avec une concentration importante des ménages entre 5 et 13. Certains ménages présentent des scores très élevés pouvant dépasser 25, révélant l’existence de situations de vulnérabilité alimentaire plus sévères au sein de l’observatoire.
:::
::::


```{r g7_csi_boxplot}
#| include: false
# G7 : Distribution du score CSI par observatoire
csi_expanded <- res_ssa %>%
  left_join(res_deb, by = "j5") %>%
  expand_sites_for_profile() %>%
  mutate(csi_score = rowSums(across(all_of(strat_vars), ~replace_na(., 0))))

n_obs <- n_distinct(csi_expanded$Observatory)

ggplot(csi_expanded, aes(x = Observatory, y = csi_score, fill = Observatory)) +
  geom_boxplot(show.legend = FALSE) +
  { if (n_obs > 1) facet_wrap(~Observatory, scales = "free_x") } +
  labs(
    # title = "Distribution du score CSI simplifié par observatoire",
    x = NULL,
    y = "Score CSI"
  ) +
  theme_ror() +
  theme(plot.title = element_text(face = "bold", size = 12))
```

```{r fig_csi_boxplot}
#| echo: false
#| label: fig-csi-boxplot
#| fig-cap: "Distribution du score CSI simplifié par observatoire.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
ggplot(csi_expanded, aes(x = Observatory, y = csi_score, fill = Observatory)) +
  geom_boxplot(show.legend = FALSE) +
  { if (n_obs > 1) facet_wrap(~Observatory, scales = "free_x") } +
  labs(
    # title = "Distribution du score CSI simplifié par observatoire",
    x = NULL,
    y = "Score CSI"
  ) +
  theme_ror() +
  theme(plot.title = element_text(face = "bold", size = 12))
```

```{r dl_g7_csi_boxplot}
#| include: false
dl_fig_variants("fig-csi-boxplot", "09", function(d) {
  sv <- c("ssa01", "ssa02", "ssa03", "ssa04", "ssa05", "ssa07",
          "ssa08", "ssa09", "ssa10", "ssa11", "ssa12", "ssa14",
          "ssa15", "ssa17")
  d <- d %>%
    mutate(csi_score = rowSums(across(all_of(sv), ~replace_na(., 0))))
  ggplot(d, aes(x = Observatory, y = csi_score, fill = Observatory)) +
    geom_boxplot(show.legend = FALSE) +
    labs(x = "Observatoire", y = "Score CSI") +
    theme_ror()
}, ssa_full)
```

::: {.content-visible when-profile="consolidated"}

## Évolution de la sécurité alimentaire (1995–2025) {#sec-sa-multiyear}

La durée de la période de soudure est disponible pour un nombre significatif de campagnes. On retrace ici son évolution pour les deux observatoires.

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
À l'Alaotra, la durée moyenne de la soudure a débuté à environ 3 mois en 1999 avant de connaître une progression importante jusqu'à un sommet de près de 6 mois en 2004. Durant la décennie suivante, entre 2005 et 2014, les valeurs sont restées relativement élevées, fluctuant généralement entre 4.5 et 5.5 mois. La campagne 2025 marque une amélioration par rapport à cette période historique avec une durée moyenne qui s'établit à 3.8 mois par ménage.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
A Marovoay présentait une situation de départ très difficile en 1995 avec une durée moyenne de soudure dépassant les 8.5 mois. Une réduction drastique a été observée au cours des années suivantes pour atteindre environ 4 mois en 2003. Entre 2004 et 2014, la durée a connu des variations cycliques comprises entre 3.5 et 5 mois environ. En 2025, la durée moyenne de la soudure atteint son niveau le plus bas enregistré depuis le début du suivi avec 2.2 mois par ménage.
:::
::::

```{r sa_trends_compute}
#| include: false

sa_file <- "output/sa_trends_obs.rds"

if (file.exists(sa_file)) {
  sa_trends <- readRDS(sa_file)
} else {
  years_dir <- "data/ROS_MDG_microdata"
  all_years <- c(1995:2015, 2025)
  
  sa_trends <- map_dfr(all_years, function(yr) {
    f_deb <- file.path(years_dir, yr, "res_deb.dta")
    f_sa  <- file.path(years_dir, yr, "res_sa.dta")
    if (!file.exists(f_deb) || !file.exists(f_sa)) return(tibble())
    
    deb <- read_dta(f_deb) |>
      mutate(j5 = as.character(j5), year = yr) |>
      filter(j0 %in% c(3, 21)) |>
      mutate(Observatory = if_else(j0 == 3, "Marovoay", "Alaotra"))
    
    sa <- read_dta(f_sa) |> mutate(j5 = as.character(j5))
    
    if (!"sal2d" %in% names(sa)) return(tibble())
    
    d <- sa |>
      inner_join(deb |> select(j5, Observatory), by = "j5") |>
      select(-any_of("year")) |>
      mutate(year = yr)

    d |>
      filter(!is.na(sal2d), sal2d >= 0, sal2d <= 12) |>
      summarise(
        n = n(),
        soudure_moy = round(mean(sal2d, na.rm = TRUE), 1),
        pct_soudure = round(mean(sal2d > 0, na.rm = TRUE) * 100, 1),
        .by = c(year, Observatory)
      )
  })
  
  saveRDS(sa_trends, sa_file)
}

sa_solid <- sa_trends |> filter(year <= 2015)
sa_gap   <- sa_trends |> filter(year %in% c(2015, 2025))
```

```{r fig_sa_soudure_duration}
#| label: fig-sa-soudure-duration
#| fig-cap: "Évolution de la durée moyenne de soudure (mois/ménage).<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-sa-soudure-duration")
#| fig-height: !expr fig_h("fig-sa-soudure-duration")

sa_trends |>
  filter(year >= 2004) |>
  make_trend_plot(y_var = soudure_moy, y_label = "Durée moyenne soudure (mois)", gap_year = 2015)
```

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
À l'Alaotra, la proportion de ménages ayant connu une période de soudure a marqué l'histoire de l'observatoire par des variations importantes. En 1999, ce taux était de 87.8% avant d'atteindre un sommet historique de 96.8% en 2003. Une tendance à la baisse s'est ensuite manifestée durant la décennie suivante, le pourcentage de ménages touchés tombant à son niveau le plus bas en 2013 avec 51.1%. Toutefois, les données de la campagne 2025 indiquent une dégradation de la situation, puisque 100% des ménages déclarent désormais subir au moins un mois de soudure au cours de l'année.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
À Marovoay, la période de soudure était quasi généralisée au début du suivi avec un taux de 99.2% des ménages en 1995. Cette situation de crise s'est maintenue à des niveaux très élevés (supérieurs à 90%) jusqu'en 2003. Une réduction significative a été enregistrée par la suite atteignant un point bas de 51.7% en 2008, avant de remonter et de fluctuer entre 58% et 72% jusqu'en 2014. Pour la campagne 2025, la prévalence de la soudure repart à la hausse et concerne 97.3% des ménages de l'observatoire.
:::
::::

```{r fig_sa_pct_soudure}
#| label: fig-sa-pct-soudure
#| fig-cap: "Évolution du pourcentage de ménages en soudure.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-sa-pct-soudure")
#| fig-height: !expr fig_h("fig-sa-pct-soudure")

sa_trends |>
  filter(year >= 2004) |>
  make_trend_plot(y_var = pct_soudure, y_label = "% ménages en soudure", gap_year = 2015)
```

:::

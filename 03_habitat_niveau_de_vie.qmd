...
df_solar_status <- res_h |>
  left_join(obs_site, by = "j5") |>
  filter(h7c == 1, !is.na(h7c1), !is.na(Observatory)) |>
  mutate(
    En_marche = if_else(h7c1 == 1, "Oui", "Non"),
    En_marche = factor(En_marche, levels = c("Oui", "Non")),
    Observatory = factor(Observatory, levels = c("Marovoay", "Alaotra"))
  ) |>
  count(Observatory, En_marche, .drop = FALSE) |>
  group_by(Observatory) |>
  mutate(pct = 100 * n / sum(n)) |>
  ungroup()

ggplot(df_solar_status, aes(x = pct, y = En_marche, fill = Observatory)) +
  geom_col(width = 0.85, show.legend = FALSE) +
  geom_text(aes(label = round(pct, 0)), hjust = -0.15, size = 5.2) +
  facet_wrap(~Observatory, nrow = 1) +
  scale_y_discrete(limits = rev(levels(df_solar_status$En_marche))) +
  scale_x_continuous(
    limits = c(0, max(df_solar_status$pct, na.rm = TRUE) + 10),
...
#| fig-cap: "Evolution du revenu total reel moyen par tete (Ar constants 2010, IPC)"
#| fig-width: !expr fig_w("fig_trends_revtot_real_pc")
#| fig-height: !expr fig_h("fig_trends_revtot_real_pc")

trends |> make_trend_plot(revtot_real_pc, "Revenu total reel moyen (Ar/tete/an, constants 2010)")
```

```{r fig_trends_revtot_real_eq}
#| fig-cap: "Evolution du revenu total reel moyen par unite de consommation (Ar constants 2010, IPC)"
#| fig-width: !expr fig_w("fig_trends_revtot_real_eq")
#| fig-height: !expr fig_h("fig_trends_revtot_real_eq")

trends |> make_trend_plot(revtot_real_eq, "Revenu total reel moyen (Ar/UC/an, constants 2010)")
```
...
#| fig-cap: "Evolution du revenu courant reel moyen par tete (Ar constants 2010, IPC)"
#| fig-width: !expr fig_w("fig_trends_revcou_real_pc")
#| fig-height: !expr fig_h("fig_trends_revcou_real_pc")

trends |> make_trend_plot(revcou_real_pc, "Revenu courant reel moyen (Ar/tete/an, constants 2010)")
```

```{r fig_trends_revcou_real_eq}
#| fig-cap: "Evolution du revenu courant reel moyen par unite de consommation (Ar constants 2010, IPC)"
#| fig-width: !expr fig_w("fig_trends_revcou_real_eq")
#| fig-height: !expr fig_h("fig_trends_revcou_real_eq")

trends |> make_trend_plot(revcou_real_eq, "Revenu courant reel moyen (Ar/UC/an, constants 2010)")
```

...
val_par_type |>
  ggplot(aes(x = reorder(Type, val_moy), y = val_moy / 1000)) +
  geom_col(fill = "#2c7bb6", show.legend = FALSE) +
  coord_flip() +
  facet_wrap(~Observatory, scales = "free_y") +
  labs(x = NULL, y = "Valeur moyenne (milliers Ar)") +
...
res_t2_obs |>
  ...
  mutate(pct = round(n / sum(n) * 100, 1), .by = Observatory) |>
  ggplot(aes(x = reorder(Destination, pct), y = pct)) +
  geom_col(fill = "#2c7bb6", show.legend = FALSE) +
  coord_flip() +
  facet_wrap(~Observatory, scales = "free_y") +
  labs(x = NULL, y = "% des transferts") +
  theme_minimal(base_size = 12)
```

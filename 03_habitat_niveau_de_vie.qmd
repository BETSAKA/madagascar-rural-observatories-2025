...
#| fig-cap: " Statut d'occupation du logement.<br> Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-height: !expr fig_h("fig-statut-logement")
#| fig-width: !expr fig_w("fig-statut-logement")
house |> make_bar_obs(x = Statut, title = NULL)
```
...
```{r}
#| label: fig-latrine
#| fig-cap: "Types de latrine. <br> Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-height: !expr fig_h("fig-latrine")
#| fig-width: !expr fig_w("fig-latrine")
latrine |> make_bar_obs(title = NULL)
```
...
dl_fig_variants("fig_latrine", "05", function(d) {
  d |> mutate(Type = decode_labelled(h5)) |>
    ...
    count(Observatory, Type) |>
    mutate(pct = round(n / sum(n) * 100, 1), .by = Observatory) |>
    make_bar_obs(title = "Types de latrine")
}, res_h |> left_join(obs, by = "j5"))
```
...
```{r}
#| label: fig-eau
#| fig-cap: "Mode d'approvisionnement en eau. <br> Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-height: !expr fig_h("fig-eau")
#| fig-width: !expr fig_w("fig-eau")
water |> make_bar_obs(title = NULL)
```
...
```{r fig_lighting}
#| label: fig-eclairage
#| fig-cap: "Modes d'éclairage.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-height: !expr fig_h("fig-eclairage")
#| fig-width: !expr fig_w("fig-eclairage")
light |> make_bar_obs(title = NULL)
```
...
```{r fig_solar_panel_status}
#| label: fig-solar-panel-status
#| fig-cap: "Panneau solaire en état de marche (parmi les utilisateurs du solaire).<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-height: !expr fig_h("fig-solar-panel-status")
#| fig-width: !expr fig_w("fig-solar-panel-status")
df_solar_plot <- solar_users |>
  count(Observatory, En_marche = if_else(h7c1 == 1, "Oui", "Non")) |>
  mutate(pct = round(n / sum(n) * 100, 1), .by = Observatory)

ggplot(df_solar_plot, aes(x = pct, y = En_marche, fill = Observatory)) +
  geom_col(width = 0.6, show.legend = FALSE) +
  geom_text(aes(label = round(pct, 0)), hjust = -0.3, size = 4) +
  facet_wrap(~Observatory, nrow = 1) +
  scale_x_continuous(labels = function(x) paste0(x, "%"), limits = c(0, 100)) +
  scale_fill_manual(values = c("Alaotra" = "#2E5090", "Marovoay" = "#2E5090")) +
  labs(x = "%", y = NULL) +
...
```{r fig-cooking}
#| label: fig-cooking
#| fig-cap: "Mode de cuisson par observatoire.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-height: !expr fig_h("fig-cooking")
cooking |> make_bar_obs()
```
...
```{r fig_comfort}
#| label: fig-comfort
#| fig-cap: "Biens de confort courants.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-height: !expr fig_h("fig-comfort")
comfort |> make_bar_obs()
```
...
```{r fig_table_equip}
#| label: fig-table-equip
#| fig-cap: "Équipements agricoles par observatoire.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-height: !expr fig_h("fig-table-equip")
table_equip |>
  make_bar_obs(x = Equipement, y = pct_menages, ncol = 2)
```
...
```{r fig_comm}
#| label: fig-comm
#| fig-cap: "Matériels de communication.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
À Alaotra, environ 33,5 % des ménages participent à l’envoi de transferts. Les transferts en riz atteignent une valeur moyenne de 97 000 Ar, suivis des transferts en argent avec une moyenne de 79 000 Ar.
:::
::::
...
# Actual labels from data: 0=Argent, 1=Riz, 2=Autres aliments, etc.
type_transf_labs <- c(
  "0" = "Argent",
  "1" = "Riz",
...
```{r fig_transferts_sortants}
#| label: fig-transferts-sortants
#| fig-cap: "Valeur moyenne des transferts sortants par observatoire et type.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
À Alaotra, environ un tiers des ménages enquêtés (33,5 %) participent à l’envoi de transferts.
:::
::::
...
#| tbl-cap: "Transferts sortants : Résumé par observatoire"
...
:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
À Alaotra, la grande majorité des destinataires des transferts résident dans la même commune avec une proportion de 89 %. Les destinataires résidant dans une autre commune de la même région constituent 6,4 %, tandis que les transferts destinés vers une autre région représentent 4,7 %.
:::
::::
...
```{r fig_destination_transferts}
#| label: fig-destination-transferts
#| fig-cap: "Lieu de résidence des destinataires de transferts.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
dl_fig_variants("fig_destination", "05", function(d) {
  ...
  d |> filter(!is.na(t41)) |>
    mutate(Destination = if_else(
      as.character(as.integer(t41)) %in% names(dest_labs),
      dest_labs[as.character(as.integer(t41))], "Non précise")) |>
    count(Observatory, Destination) |>
    mutate(pct = round(n / sum(n) * 100, 1), .by = Observatory) |>
...
Pour les deux observatoires, le fait que la moyenne soit nettement supérieure à la médiane (environ 66 % de plus à Marovoay et 67 % à l'Alaotra) confirme qu'une minorité de ménages disposant de revenus très élevés tire la moyenne vers le haut, illustrant les inégalités de richesse dans ces bassins rizicoles. Aussi, le revenu par équivalent est supérieur au revenu par tête, ce qui montre que l'ajustement pour la composition du ménage améliore le niveau de vie économique estimé des membres du ménage.
L'Alaotra présente des niveaux de vie nettement supérieurs à ceux de Marovoay, quelle que soit l'unité de mesure choisie.
:::

:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
À Alaotra, le revenu total annuel moyen par ménage est de 4 488 029 Ar. La moitié des ménages ont des revenu au maximum 2 692 468 Ar. Chaque individu d'un ménage de l'observatoire d'Alaotra dispose d'un revenu annuel de 1.224.262 Ar, soit environ 102.022 Ar par mois.
En tenant compte de la taille et de la composition des ménages selon l'échelle d'équivalence de l'OCDE modifiée, le revenu annuel moyen est de 2.011.838 Ar par unité de consommation, soit environ 167.653 Ar par mois. 
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
À Marovoay, le revenu annuel moyen par ménage est de 3 480 646 Ar et la médiane est de 2 094 749 Ar. L’écart-type est de 5 475 598 Ar indiquant une variation importante des revenus au sein des ménages enquêtés.
Chaque individu dispose en moyenne d'un revenu annuel de 870.716 Ar, soit environ 72.560 Ar par mois.
Après ajustement selon l'échelle d'équivalence de l'OCDE modifiée, le revenu annuel moyen atteint 1.473.963 Ar par unité de consommation, soit environ 122.830 Ar par mois. 
:::
::::

```{r tbl_revtot_obs}
#| tbl-cap: "Revenu total annuel (Ar/an)"
#| tbl-cap-location: top
#| label: tbl-revtot-obs
inc |>
  summarise(
    `Nb ménages` = n(),
...
#   ) |>
#   fmt_number(columns = Moyenne: `Écart-type`, use_seps = TRUE, decimals = 0) |>
#   style_table()
```
...
:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, les revenus nets rizicoles constituent la principale composante du revenu courant moyen par ménage avec 38 % du total, soit 1 654 368 Ar/an. Les revenus indépendants arrivent en deuxième position avec 24.0 %, représentant 1 043 037 Ar/an. Les salaires agricoles occupent la troisième place avec 19.8 %, soit 860 748 Ar/an.L’élevage contribue à hauteur de 7% du revenu courant moyen, correspondant à 302 735 Ar/an. Les salaires non agricoles représentent 6.8 %, soit 297 052 Ar/an. Les revenus nets issus des autres cultures participent pour 4.4 %, équivalant à 191 317 Ar/an.  Le revenu courant moyen total par ménage dans l’observatoire d’Alaotra est de 4 349 257 Ar/an.
:::
::::
...
```{r fig_revcou_shares}
#| label: fig-revcou-shares
#| fig-cap: "Part des composantes dans le revenu courant moyen par observatoire.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-revcou-shares")
#| fig-height: !expr fig_h("fig-revcou-shares")
...
dl_fig_variants("fig-revcou-shares", "05", function(d) {
  d |> summarise(
    `Salaires agricoles` = mean(revsec, na.rm = TRUE),
    `Salaires non-agricoles` = mean(revppal, na.rm = TRUE),
    `Revenus independants` = mean(rev_indep, na.rm = TRUE),
...
```{r fig_revtot_dist}
#| label: fig-revtot-dist
#| fig-cap: "Distribution du revenu total par observatoire (echelle log).<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-revtot-dist")
#| fig-height: !expr fig_h("fig-revtot-dist")
...
```{r tbl_revcou_revexcept}
#| tbl-cap: "Revenu courant et revenu exceptionnel moyens par menage (Ar/an)"
#| tbl-cap-location: top
#| label: tbl-revcou-revexcept
inc |>
  summarise(
...
Les observatoires de Marovoay et d'Alaotra disposent de series longues permettant de suivre l'évolution des revenus sur trois decennies. Les donn2es couvrent 1995-2008 et 2010-2014 pour Marovoay, 1999-2008 et 2010-2014 pour Alaotra, ainsi que la campagne 2025 pour les deux. Les années 2009 et 2015-2024 ne contiennent pas ces observatoires. Le trait pointillé matérialise l'absence de données.
...
# Recompute derived real-income series even when loading a cached RDS,
# so newly added columns are always available during render.
if (!"cpi" %in% names(trends)) {
  library(jsonlite)
  wb_raw <- fromJSON(
    "https://api.worldbank.org/v2/country/MDG/indicator/FP.CPI.TOTL?date=1995:2025&format=json&per_page=50"
  )
  ...
  trends <- trends |> left_join(cpi_full, by = "year")
}

trends <- trends |>
  mutate(
    deflator       = cpi / 100,
    revcou_real_pc = (revcou_mean / hh_size_mean) / deflator,
    revcou_real_eq = (revcou_mean / eq_oecd_mean) / deflator,
...
```{r fig_trends_revtot}
#| label: fig-trends-revtot
#| fig-cap: "Evolution du revenu total moyen par menage (Ar courants.<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-trends-revtot")
#| fig-height: !expr fig_h("fig-trends-revtot")
...
```{r fig_trends_revcou}
#| label: fig-trends-revcou
#| fig-cap: "Evolution du revenu courant moyen par menage (Ar courants).<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-trends-revcou")
#| fig-height: !expr fig_h("fig-trends-revcou")
...
```{r fig_trends_decomp}
#| label: fig-trends-decomp
#| fig-cap: "Evolution des composantes du revenu courant moyen (Ar courants).<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-trends-decomp")
#| fig-height: !expr fig_h("fig-trends-decomp")
...
```{r fig_ipc_national}
#| label: fig-ipc-national
#| fig-cap: "Evolution de l'IPC national Madagascar (base 2010 = 100).<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-ipc-national")
#| fig-height: !expr fig_h("fig-ipc-national")
...
Le prix de vente median du paddy, calcule a partir des ventes declarees par les menages, fournit un indicateur complementaire de l'inflation locale.
...
```{r fig_trends_paddy}
#| label: fig-trends-paddy
#| fig-cap: "Evolution du prix median du paddy (Ar/kg).<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-trends-paddy")
#| fig-height: !expr fig_h("fig-trends-paddy")
...
```{r fig_ipc_vs_paddy}
#| label: fig-ipc-vs-paddy
#| fig-cap: "Comparaison de l'IPC national et de l'indice du prix du paddy (base 100 en 2010).<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-ipc-vs-paddy")
#| fig-height: !expr fig_h("fig-ipc-vs-paddy")
...
#| label: fig-trends-revtot-real
#| fig-cap: "Evolution du revenu total réel moyen par ménage (Ar constants 2010, IPC).<br>Source: Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-trends-revtot-real")
#| fig-height: !expr fig_h("fig-trends-revtot-real")

trends |> make_trend_plot(revtot_real, "Revenu total réel moyen (Ar constants 2010)")
```

```{r fig_trends_revtot_real_pc}
#| label: fig-trends-revtot-real-pc
#| fig-cap: "Evolution du revenu total réel moyen par tête (Ar constants 2010, IPC).<br>Source: Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-trends-revtot-real-pc")
#| fig-height: !expr fig_h("fig-trends-revtot-real-pc")

trends |> make_trend_plot(revtot_real_pc, "Revenu total réel moyen (Ar/tete/an, constants 2010)")
```

```{r fig_trends_revtot_real_eq}
#| label: fig-trends-revtot-real-eq
#| fig-cap: "Evolution du revenu total réel moyen par unité de consommation (Ar constants 2010, IPC).<br>Source: Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig_trends_revtot_real_eq")
#| fig-height: !expr fig_h("fig_trends_revtot_real_eq")

trends |> make_trend_plot(revtot_real_eq, "Revenu total réel moyen (Ar/UC/an, constants 2010)")
```
...
#| label: fig-trends-revcou-real
#| fig-cap: "Evolution du revenu courant réel moyen par ménage (Ar constants 2010, IPC).<br>Source: Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-trends-revcou-real")
#| fig-height: !expr fig_h("fig-trends-revcou-real")

trends |> make_trend_plot(revcou_real, "Revenu courant reél moyen (Ar constants 2010)")
```

```{r fig_trends_revcou_real_pc}
#| label: fig-trends-revcou-real-pc
#| fig-cap: "Evolution du revenu courant réel moyen par tête (Ar constants 2010, IPC).<br>Source: Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig_trends_revcou_real_pc")
#| fig-height: !expr fig_h("fig_trends_revcou_real_pc")

trends |> make_trend_plot(revcou_real_pc, "Revenu courant réel moyen (Ar/tête/an, constants 2010)")
```

```{r fig_trends_revcou_real_eq}
#| label: fig-trends-revcou-real-eq
#| fig-cap: "Evolution du revenu courant réel moyen par unité de consommation (Ar constants 2010, IPC).<br>Source: Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-trends-revcou-real-eq")
#| fig-height: !expr fig_h("fig-trends-revcou-real-eq")

trends |> make_trend_plot(revcou_real_eq, "Revenu courant réel moyen (Ar/UC/an, constants 2010)")
```

### Revenus réels (deflatés par le prix du paddy)

Le prix du paddy, produit central de ces economies rizicoles, offre un déflateur alternatif plus proche de la realite locale que l'IPC national. Les revenus sont ici exprimes en equivalent-paddy a prix 2010, c'est-a-dire divises par l'indice du prix median du paddy de chaque observatoire (base 2010 = 1). Ce déflateur est propre a chaque observatoire. 

Le revenu total réel moyen par ménage exprimé en constants paddy 2010 présente de fortes fluctuations dans les deux observatoires au cours de la période étudiée. Cette approche par le déflateur-paddy est d'autant plus cruciale qu'elle met en lumière un appauvrissement réel et structurel des ménages au cours de la dernière décennie. En effet, la baisse des revenus exprimés en équivalent-paddy traduit une érosion directe du pouvoir d'achat sur le bien le plus vital de ces économies ruraux.
...
#| label: fig-trends-revtot-paddy
#| fig-cap: "Evolution du revenu total réel moyen par ménage (Ar constants paddy 2010).<br>Source: Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-trends-revtot-paddy")
#| fig-height: !expr fig_h("fig-trends-revtot-paddy")

trends |> make_trend_plot(revtot_paddy, "Revenu total réel moyen (Ar constants paddy 2010)")
```
...
#| label: fig-trends-revcou-paddy
#| fig-cap: "Evolution du revenu courant réel moyen par ménage (Ar constants paddy 2010).<br>Source: Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-trends-revcou-paddy")
#| fig-height: !expr fig_h("fig-trends-revcou-paddy")

trends |> make_trend_plot(revcou_paddy, "Revenu courant réel moyen (Ar constants paddy 2010)")
```

## Evolution des conditions d'habitat (1997-2025) {#sec-habitat-multiyear}

Les donnees sur l'habitat permettent de suivre l'evolution de la source d'eau principale sur la periode couverte.
...
```{r fig_habitat_water}
#| label: fig-habitat-water
#| fig-cap: "Evolution de l'accès à une source d'eau ameliorée (1997-2025).<br>Source: Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-habitat-water")
#| fig-height: !expr fig_h("fig-habitat-water")

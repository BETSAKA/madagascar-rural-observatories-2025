...
::: obs-alaotra
A Alaotra, seulement 32.1 % des ménages sont membres dans une organisation ou association. 
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
A Marovoay, une majorité de 55.3 % des ménages participent à la vie associative.
:::
::::

```{r tbl_adhesion}
#| label: tbl-adhesion
#| tbl-cap: "Ménages ayant au moins un membre dans une organisation"
...
```{r fig_nb_org}
#| label: fig-nb-org
#| fig-cap: "Nombre d'organisations par ménage membre. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_type_org}
#| label: fig-type-org
#| fig-cap: "Type d'organisation (toutes adhésions). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| fig-cap: "Membre du ménage affilié à l'organisation. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| echo: false
#| results: asis
dl_fig_variants("fig-qui-membre", "12", function(d) {
  d |>
    filter(!is.na(respgc)) |>
    mutate(Membre = case_when(
      respgb == 1 ~ "Chef de ménage",
...
```{r fig_responsabilite}
#| label: fig-responsabilite
#| fig-cap: "Niveau de responsabilité dans l'organisation. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| echo: false
#| results: asis
dl_fig_variants("fig-responsabilite", "12", function(d) {
  d |>
    filter(!is.na(respgd), respgd %in% c(1, 2, 3)) |>
    mutate(Role = factor(as.character(respgd), levels = names(role_labs), labels = role_labs)) |>
    count(Observatory, Role) |>
...
::: {.content-hidden when-profile="marovoay"}
::: {.obs-alaotra}
Dans l'Alaotra, un ménage sur cinq  déclare avoir bénéficié d'au moins un projet ou d'une formation. 
:::
:::
...
#| echo: false
#| label: fig-pro1
#| fig-cap: "Ménages bénéficiaires de projets ou formations. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
res_respg0 |>
  filter(!is.na(pro1)) |>
  expand_sites_for_profile() |>
...
#| echo: false
#| results: asis
dl_fig_variants("fig-pro1", "12", function(d) {
  d |>
    filter(!is.na(pro1)) |>
    mutate(Type = case_when(
      pro1 == 1 ~ "Oui", pro1 == 2 ~ "Non", pro1 == 3 ~ "Ne sait pas"
...
#| fig-cap: "Taux de ménages bénéficiaires par type de projet. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| echo: false
#| results: asis
dl_fig_variants("fig-benef-type", "12", function(d) {
  d |>
    filter(!is.na(pro2), pro2 %in% c(1, 2)) |>
...
```{r prep_pro_benef}
#| include: false
# Préparation commune : ménages bénéficiaires
pro_benef <- pro_clean |>
  filter(pro2 == 1) |>
  expand_sites_for_profile()
```
...
#| tbl-cap: "Source des projets pour les ménages bénéficiaires (tous types confondus)"
...
#| fig-cap: "Source des projets pour les ménages bénéficiaires. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| fig-cap: "Accès aux projets selon l'appartenance à une organisation. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
res_respg0 |>
  filter(pro1 %in% c(1, 2)) |>
  mutate(
...
#| echo: false
#| results: asis
dl_fig_variants("fig-croisement", "12", function(d) {
  d |>
    filter(pro1 %in% c(1, 2)) |>
    mutate(
      Type = if_else(respga == 1, "Membre", "Non membre"),

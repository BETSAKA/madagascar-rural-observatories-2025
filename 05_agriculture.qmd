...
pct_vente_a <- res_dc20 |> filter(j5 %in% res_r1$j5, Observatory == "Alaotra") |>
  summarise(p = round(mean(dcva == 1, na.rm = TRUE) * 100, 1)) |> pull(p)
pct_vente_m <- res_dc20 |> filter(j5 %in% res_r1$j5, Observatory == "Marovoay") |>
  summarise(p = round(mean(dcva == 1, na.rm = TRUE) * 100, 1)) |> pull(p)
...
#| fig-cap: "Mode de faire valoir des parcelles rizicoles par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_eau_riz}
#| label: fig-eau-riz
#| fig-cap: "Problèmes de maîtrise de l'eau sur les parcelles rizicoles. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| fig-cap: "Technique de culture rizicole par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| fig-cap: "Utilisation d'engrais sur les parcelles rizicoles. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| echo: false
#| label: fig-semences-labour
#| fig-cap: "Mode de travail du sol sur les parcelles rizicoles. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-height: !expr fig_h("fig-semences-labour")
res_r |>
  filter(!is.na(r14)) |>
  mutate(
...
#| echo: false
#| results: asis
dl_fig_variants("fig-semences-labour", "08", function(d) {
  d |>
    filter(!is.na(r14)) |>
    mutate(
      Type = factor(as.character(r14),
...
#| echo: false
#| label: fig-rendement-perception
#| fig-cap: "Perception des rendements rizicoles. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
res_r1 |>
...
#| echo: false
#| results: asis
dl_fig_variants("fig-rendement-perception", "08", function(d) {
  d |>
    filter(r1a == 1, !is.na(dc23c6)) |>
    mutate(Type = safe_to_char(dc23c6)) |>
    count(Observatory, Type) |>
...
```{r fig_rendement_riz_tha}
#| label: fig-rendement-riz-tha
#| fig-cap: "Distribution des rendements rizicoles (t/ha) par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_topographie_riz}
#| label: fig-topographie-riz
#| fig-cap: "Topographie des parcelles rizicoles. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| fig-cap: "Saison de culture rizicole. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_origine_semence}
#| label: fig-origine-semence
#| fig-cap: "Origine des semences de riz. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
::: {.content-hidden when-profile="marovoay"}
::: {.obs-alaotra}
Dans l'Alaotra, `r pct_vente_a` % des ménages riziculteurs déclarent avoir vendu du riz au cours de la campagne.
:::
:::

::: {.content-hidden when-profile="alaotra"}
::: {.obs-marovoay}
A Marovoay, `r pct_vente_m` % des ménages riziculteurs déclarent avoir vendu du riz au cours de la campagne.
:::
:::

```{r fig_ventes_riz_taux}
#| label: fig-ventes-riz-taux
#| fig-cap: "Taux de vente de riz par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-ventes-riz-taux")
#| fig-height: !expr fig_h("fig-ventes-riz-taux")

res_dc20 |>
  filter(j5 %in% res_r1$j5) |>
  mutate(Vente = if_else(dcva == 1, "Vendu", "Non vendu")) |>
  count(Observatory, Vente) |>
  mutate(pct = round(n / sum(n) * 100, 1), .by = Observatory) |>
  expand_sites_for_profile() |>
  make_bar_obs(x = Vente, y_label = "% des riziculteurs")
```

```{r dl_fig_ventes_riz_taux}
#| echo: false
#| results: asis
dl_fig_variants("fig_ventes_riz_taux", "08", function(d) {
  d |>
    filter(j5 %in% res_r1$j5) |>
    mutate(Vente = if_else(dcva == 1, "Vendu", "Non vendu")) |>
    count(Observatory, Vente) |>
...
```{r fig_ventes_riz_mois}
#| label: fig-ventes-riz-mois
#| fig-cap: "Répartition mensuelle des ventes de riz. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| fig-cap: "Distribution du prix de vente du paddy (Ar/kg). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| echo: false
#| label: fig-bilan-riz
#| fig-cap: "Bilan rizicole moyen par ménage (kg de paddy). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
res_dc1 |>
  ...
  pivot_longer(-Observatory, names_to = "Variable", values_to = "kg") |>
  mutate(
    Poste = bilan_vars[Variable],
    Flux = if_else(Variable %in% c("dc01", "dc02b", "dc03b", "dc04", "dc05"), "Ressources", "Emplois"),
    Flux = factor(Flux, levels = c("Ressources", "Emplois"))
  ) |>
  ror_bar_stacked(x = Flux, y = kg, fill = Poste, 
                  proportion = FALSE, direction = "vertical", show_pct = FALSE,
                  y_label = "kg de paddy") +
geom_text(
  aes(label = if_else(kg >= 30, as.character(round(kg, 0)), "")),
...
#| echo: false
#| results: asis
dl_fig_variants("fig-bilan-riz", "08", function(d) {
  bv <- bilan_vars[names(bilan_vars) %in% names(d)]
  d |>
    summarise(
      across(all_of(names(bv)), \(x) round(mean(x, na.rm = TRUE))),
...
#| tbl-cap: "Pratique des cultures non rizicoles (% ménages)"
...
```{r fig_diversification}
#| label: fig-diversification
#| fig-cap: "Nombre de cultures non rizicoles par ménage. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| fig-cap: "Cultures non rizicoles les plus fréquentes (nb de ménages pratiquant). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_engrais_autres}
#| label: fig-engrais-autres
#| fig-cap: "Utilisation d'engrais sur les parcelles de cultures non rizicoles. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_rendement_perception_autres}
#| label: fig-rendement-perception-autres
#| fig-cap: "Perception des rendements des cultures non rizicoles. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-rendement-perception-autres")
#| fig-height: !expr fig_h("fig-rendement-perception-autres")
res_c0 |>
  ...
  mutate(pct = round(n / sum(n) * 100, 1), .by = Observatory) |>
  expand_sites_for_profile() |>
  make_bar_obs(x = Perception, y_label = "% des pratiquants")
```

```{r dl_fig_rendement_perception_autres}
#| echo: false
#| results: asis
dl_fig_variants("fig_rendement_perception_autres", "08", function(d) {
  d |>
    filter(c1a == 1, !is.na(c20)) |>
    mutate(Perception = safe_to_char(c20)) |>
    count(Observatory, Perception) |>
...
```{r fig_rendement_evolution_autres}
#| label: fig-rendement-evolution-autres
#| fig-cap: "Évolution des rendements des cultures non rizicoles par rapport à l'an dernier. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-rendement-evolution-autres")
#| fig-height: !expr fig_h("fig-rendement-evolution-autres")
res_c0 |>
  ...
  mutate(pct = round(n / sum(n) * 100, 1), .by = Observatory) |>
  expand_sites_for_profile() |>
  make_bar_obs(x = Evolution, y_label = "% des pratiquants")
```

```{r dl_fig_rendement_evolution_autres}
#| echo: false
#| results: asis
dl_fig_variants("fig_rendement_evolution_autres", "08", function(d) {
  d |>
    filter(c1a == 1, !is.na(c21)) |>
    mutate(Evolution = safe_to_char(c21)) |>
    count(Observatory, Evolution) |>
...
```{r tbl_jardin_pratique}
#| label: tbl-jardin-pratique
#| tbl-cap: "Pratique du jardin potager et de la cueillette (% ménages)"
...
#| fig-cap: "Produits de jardin / cueillette les plus fréquents. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r pratique_foret_data}
#| include: false
pratique_foret <- res_jpchphef |>
  summarise(
    pct = round(sum(efa == 1, na.rm = TRUE) / n() * 100, 1),
    .by = Observatory
  ) |>
  mutate(Type = "Pratiquent") |>
  expand_sites_for_profile()
```

```{r fig_pratique_foret}
#| label: fig-pratique-foret
#| fig-cap: "Part des ménages pratiquant l'exploitation forestière. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-pratique-foret")
#| fig-height: !expr fig_h("fig-pratique-foret")

pratique_foret |>
  make_bar_obs(x = Type, y_label = "% des ménages")
```

```{r dl_fig_pratique_foret}
#| echo: false
#| results: asis
dl_fig_variants("fig_pratique_foret", "08", function(d) {
  d |>
    summarise(
      pct = round(sum(efa == 1, na.rm = TRUE) / n() * 100, 1),
      .by = Observatory
...
```{r fig_produits_foret}
#| label: fig-produits-foret
#| fig-cap: "Principaux produits de l'exploitation forestière (% des ménages pratiquant l'exploitation forestière). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-produits-foret")
#| fig-height: !expr fig_h("fig-produits-foret")

produits_foret |>
  make_bar_obs(x = Produit, y_label = "% des ménages pratiquant l'exploitation forestière")
```
...
dl_fig_variants("fig_produits_foret", "08", function(d) {
  d |>
    ...
    ungroup() |>
    ror_bar_grouped(x = Produit, y = n, fill = Observatory,
                    y_label = "Nombre de ménages pratiquant l'exploitation forestière", direction = "horizontal",
                    facet = FALSE, show_pct = FALSE)
}, res_ef)
```
...
#| tbl-cap: "Pratique de l'élevage (% ménages)"
...
:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, l’espèce la plus fréquemment élevée parmi les ménages pratiquant l’élevage est les poulets (akoho)(69.2 %). Les cochons (kisoa) arrivent ensuite avec 29.6 %, suivis des oies (gisa) (22.4 %) et des canards (gana) (16.9 %). Les bovins restent moins répandus avec 14.8 % pour les zébus de labour (omby miasa), 11.8 % pour les vaches (omby vavy). Les ovins (ondry) concernent 4.8 % des ménages pratiquants, tandis que les dindons (voron-tsiloza) demeure marginal avec 1.2 %. Les caprins (osy) et les poissons (trondro) sont absents des déclarations dans cet observatoire.
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
A Marovoay, les poulets (akoho) sont également l’espèce la plus fréquemment élevée parmi les ménages pratiquants, avec 53.5 %. Les canards (gana) occupent la deuxième position avec 44.7 %, suivis des zébus de labour (omby miasa) (36.1 %) et des cochons (kisoa) (27.3 %). Les les vaches (omby vavy) concernent 19.0 % des ménages pratiquants, tandis que les ovins (ondry) 7.5 %. Les autres espèces restent marginales, notamment les oies (gisa) (2.4 %), les dindons (voron-tsiloza) (2.1 %) et les caprins (osy) (1.3 %). Lespoissons (trondro) ne sont pas déclaré dans cet observatoire.
:::
::::
...
especes_elevage <- res_ele |>
  ...
  mutate(Present = !is.na(ele3) | !is.na(ele4)) |>
  distinct(j5, Observatory, Espece, Present) |>
  summarise(n = n_distinct(j5[Present]), .by = c(Observatory, Espece)) |>
  group_by(Espece) |>
  mutate(total = sum(n)) |>
  ungroup() |>
  expand_sites_for_profile() |>
  left_join(
    res_el2 |> filter(el2 == 1) |> distinct(j5, Observatory) |>
      expand_sites_for_profile() |> count(Observatory, name = "n_total"),
    by = "Observatory"
  ) |>
  mutate(pct = round(n / n_total * 100, 1))
...
```{r fig_especes_elevage}
#| label: fig-especes-elevage
#| fig-cap: "Espèces élevées (% des ménages pratiquant). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-especes-elevage")
#| fig-height: !expr fig_h("fig-especes-elevage")

especes_elevage |>
  make_bar_obs(x = Espece, y_label = "% des ménages pratiquant l'élevage")
```
...
:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
À Alaotra, l’épargne ou l’assurance constitue le principal objectif de l’élevage pour la plupart des espèces. Cette fonction est particulièrement marquée chez les bovins (omby) avec des proportions comprises entre 75 % et 85 %, soit 77 % pour les zébus de labour (omby miasa), 75 % pour les vaches (omby vavy). Elle est également importante pour les cochons (kisoa) (47 %), les ovins (ondry) (38 %), les volailles réparties comme suit : les dindons (voron-tsiloza) (43 %), les canards (gana) (36 %), les poulets (akoho) (34 %) et les gisa (33 %). 
La vente représente également un objectif important notamment pour les cochons (kisoa) (49 %), les ovins (ondry (41 %) ; ensuite les volailles :les dindons (voron-tsiloza) (43 %), les canards (gana) (34 %), les poulets (akoho) et les oies (gisa) (33 % chacun). 
L’autoconsommation reste plus limitée et concerne principalement les volailles : poulets (akoho) (33 %), les oies (gisa) (27 %), les dindons (voron-tsiloza) (14 %) ; ainsi que les ovins (ondry) (22 %).
:::
::::

:::: {.content-hidden when-profile="alaotra"}
::: obs-marovoay
À Marovoay, l’épargne ou l’assurance domine très largement les objectifs de l’élevage. Elle est particulièrement élevée chez les bovins (omby) avec 84 % chacun pour les zébus de labour (omby miasa) et les vaches (omby vavy). Elle concerne également les volailles : les oies (gisa) (80 %), les dindons (voron-tsiloza) (53 %), les poulets (akoho) et les canards (gana) (52 % chacun); puis les ovins (ondry (65 %), les cochons (kisoa) (64 %), et les caprins (osy) (50 %). 
La vente constitue le deuxième objectif le plus fréquent surtout pour les caprins (osy) (40 %), les volailles : canards (gana) (38 %), les poulets (akoho) (34 %), les dindons (voron-tsiloza) (27 %) ; ensuite les cochons (kisoa) (36 %),  ainsi que les ovins (ondry) (27%). 
L’autoconsommation demeure marginale mais atteint pour les volailles : 20 % dindons (voron-tsiloza), 15 % pour les poulets (akoho) ; puis 10 % pour les caprins (osy).
:::
::::

```{r fig_objectifs_elevage}
#| label: fig-objectifs-elevage
#| fig-cap: "Objectifs de l'élevage par espèce (% des ménages éleveurs). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
res_ele |>
  ...
  geom_text(
    aes(
      y = p,
      label = if_else(p >= 0.08, scales::percent(p, accuracy = 1), "")
    ),
...
:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, les pertes de bétail concernent principalement les volailles (Akoho), avec environ 160 ménages ayant subi des pertes. Les oies (Gisa) arrivent ensuite avec près de 25 ménages touchés, suivis des canards (Gana) et des cochons (Kisoa) avec environ 15 ménages chacun. Les ovins (Ondry) enregistrent des pertes plus limitées, autour de 5 ménages. Les autres espèces, notamment les zébus (Omby Miasa, Omby Vavy) et les dindons (Voron-tsilozà) , présentent des effectifs très faibles, proches de 1 à 3 ménages concernés.
:::
::::
...
```{r fig_pertes_elevage}
#| label: fig-pertes-elevage
#| fig-cap: "Ménages ayant subi des pertes de bétail. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| echo: false
#| label: fig-produits-elevage
#| fig-cap: "Produits de l'élevage déclarés par les ménages. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-height: !expr fig_h("fig-produits-elevage")
pe_plot <- res_pe |>
  filter(pe_1 != "", !is.na(pe_1)) |>
  mutate(Produit = str_to_title(pe_1)) |>
  distinct(j5, Observatory, Produit) |>
...
#| echo: false
#| results: asis
dl_fig_variants("fig-produits-elevage", "08", function(d) {
  pe_tot <- d |>
    filter(pe_1 != "", !is.na(pe_1)) |>
    distinct(j5, Observatory) |>
    count(Observatory, name = "n_total")
...
#| tbl-cap: "Pratique de la chasse et de la pêche (% ménages)"
...
```{r fig_chasse_especes}
#| label: fig-chasse-especes
#| fig-cap: "Principales espèces chassées/pêchées. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| fig-cap: "Raisons de la chasse/pêche par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_frequence_peche}
#| label: fig-frequence-peche
#| fig-cap: "Fréquence de la chasse/pêche. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_saisonnalite_peche}
#| label: fig-saisonnalite-peche
#| fig-cap: "Saisonnalité de la chasse/pêche (% de mentions par mois). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r tbl_main_oeuvre_pratique}
#| label: tbl-main-oeuvre-pratique
#| tbl-cap: "Recours à la main d'oeuvre agricole (% ménages)"
...
```{r tbl_main_oeuvre_perm}
#| label: tbl-main-oeuvre-perm
#| tbl-cap: "Caractéristiques de la main d'oeuvre permanente par type d'activités"
...
```{r fig_agro_prod_riz}
#| label: fig-agro-prod-riz
#| fig-cap: "Évolution de la production rizicole moyenne par ménage (kg/ménage/an). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-agro-prod-riz")
#| fig-height: !expr fig_h("fig-agro-prod-riz")
...
```{r fig_agro_elevage}
#| label: fig-agro-elevage
#| fig-cap: "Évolution du taux de pratique de l'élevage (1998–2025). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: !expr fig_w("fig-agro-elevage")
#| fig-height: !expr fig_h("fig-agro-elevage")
...
-   **Détail des ventes de riz** : les variables décomposent les usages du riz vendu (semence, consommation, paiement de main d'œuvre), mais leur codage est un peu complexe/à préciser.
-   **Forme des produits vendus** (cultures non rizicoles) : la variable indique si le produit est vendu brut, transformé ou autre — potentiellement utile pour un tableau croisé avec les types de culture.
-   **Coûts détaillés de la main d'œuvre par opération rizicole** : le salaire total par opération (Asa tany, Mijinja, Manetsa) pourrait être présenté sous forme de tableau comparatif, mais les valeurs présentent une forte dispersion.
-   **Pratiques de nourriture de la main d'oeuvre** : la variable indique si les salariés ou l'entraide sont nourris pendant le travail.
-   **Effectifs et variations du cheptel** : les variables permettraient de reconstituer un bilan démographique du cheptel par espèce (difficile à exploiter à première vue). -->

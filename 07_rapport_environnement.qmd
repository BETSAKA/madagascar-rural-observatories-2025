...
```{r fig_ressources_dispo}
#| label: fig-ressources-dispo
#| fig-cap: "Disponibilité perçue des ressources naturelles dans la zone protégée, par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_evolution_ressources}
#| fig-cap: "Évolution perçue des ressources naturelles au cours des 3 dernières années. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
La @fig-appartenance montre les réponses par observatoire (réponses multiples).
...
```{r fig_appartenance}
#| fig-cap: "À qui appartient la zone protégée ? (% de Oui, réponses multiples). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_menaces}
#| label: fig-menaces
#| fig-cap: "Menaces perçues et responsables identifiés (% de Oui). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-height: !expr fig_h("fig-menaces")
...
#| echo: false
#| results: asis
dl_fig_variants("fig-menaces", "10", function(d) {
  pct_v <- function(df, var, label) {
    var_sym <- sym(var)
    df |> summarise(pct = round(sum(!!var_sym == 1, na.rm = TRUE) / sum(!is.na(!!var_sym)) * 100, 0), .by = Observatory) |> mutate(Item = label)
  }
...
```{r fig_representation}
#| label: fig-representation
#| fig-cap: "Que représente la zone protégée pour les ménages ? (% de Oui). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| echo: false
#| results: asis
dl_fig_variants("fig-representation", "10", function(d) {
  rm5_items <- tribble(
    ~var, ~label,
...
<!-- Le fichier `res_ef` recense les produits forestiers collectés par les ménages : charbon (*saribao*), bois de chauffe (*kitay*) et bois d'oeuvre.  -->
La @fig-ef-produits présente le pourcentage de ménages concernés par chaque produit et la @fig-ef-origine distingue l'origine (dans ou hors aire protégée).
...
#| fig-cap: "Pourcentage de ménages pratiquant la collecte de produits forestiers, par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| fig-cap: "Origine des produits forestiers collectés (dans ou hors Aire Protégée). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: 12
#| fig-height: 8
...
ef_orig <- res_ef |>
  ...
  mutate(pct = round(n / sum(n) * 100, 0), .by = c(Observatory, Produit))

ef_orig |>
  mutate(Observatory = fct_rev(Observatory)) |>
  ggplot(aes(x = Produit, y = pct, fill = Origine)) +
  geom_col(position = "stack") +
  geom_text(aes(label = paste0(pct, "%")), position = position_stack(vjust = 0.5), size = 6, color = "black") +
  facet_wrap(~Observatory, nrow = 2) +
...
```{r fig_ef_saison}
#| label: fig-ef-saison
#| fig-cap: "Saisonnalité de la collecte de produits forestiers (% de ménages actifs par mois). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
ef_vente |>
  ...
  # fmt_number(
  #   columns = montant_moyen, decimals = 0, sep_mark = " ") |>
    tab_style(
    style = list(cell_text(weight = "bold"), cell_fill(color = "#DCDCDC")),
    locations = cells_column_labels(columns = everything())
  ) |>
...
<!-- La variable `UM1` indique si le ménage se rend dans la zone protégée.  -->
Le @tbl-frequentation résume la fréquentation et ses caractéristiques. 
...
```{r fig_non_freq}
#| label: fig-non-freq
#| fig-cap: "Raisons de ne pas se rendre dans l'Aire protégée (% de Oui parmi ceux qui n'y vont pas). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_usages_ap}
#| label: fig-usages-ap
#| fig-cap: "Raisons de se rendre dans l'Aire protégée (% de Oui parmi les usagers). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| fig-cap: "Fréquence des déplacements dans l'AP selon l'usage, par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_saison_ap}
#| label: fig-saison-ap
#| fig-cap: "Saisonnalité des usages de l'AP (% d'usagers actifs par mois, principales raisons), par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra, la principale raison perçue du recours aux feux est le nettoyage des parcelles, cité par 52 % des ménages. Cette motivation est suivie par l'augmentation des surfaces cultivées (24 %) et l'augmentation des surfaces de pâturage (15 %). Ces résultats montrent que les feux sont avant tout associés à des objectifs de préparation et d'extension des espaces de production agricole et pastorale.
:::
::::
...
#| fig-cap: "Raisons perçues de faire des feux (% de Oui). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
:::: {.content-hidden when-profile="marovoay"}
::: obs-alaotra
A Alaotra,  80 % des ménages indiquent que les feux ont été réalisés loin des aires protégées (hors AP), tandis que 20 % rapportent des feux à proximité des villages. Cette répartition montre que les feux sont majoritairement localisés à distance des zones habitées.
De plus, les ménages montrent une forte adhésion à la maîtrise des feux, 82 % déclarant chercher à les contrôler. Par ailleurs, 67 % considèrent que le recours aux feux constitue une pratique normale. En revanche, les autres indicateurs restent relativement peu fréquents : 33 % rapportent que des feux leur ont déjà échappé, 11 % déclarent avoir déjà pratiqué des feux et 10 % indiquent avoir réalisé des feux au cours des douze derniers mois.
:::
::::
...
```{r fig_feux}
#| label: fig-feux
#| fig-cap: "Pratique et perception des feux (% de Oui). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| echo: false
#| results: asis
dl_fig_variants("fig-feux", "10", function(d) {
  pct_v <- function(df, var, label) {
    var_sym <- sym(var)
    df |> summarise(pct = round(sum(!!var_sym == 1, na.rm = TRUE) / sum(!is.na(!!var_sym)) * 100, 0), .by = Observatory) |> mutate(Item = label)
  }
...
#| fig-cap: "Saisonnalité des feux de brousse pratiqués (% de ménages actifs par mois, parmi ceux qui ont fait du feu). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| echo: false
#| results: asis
dl_fig_variants("fig-saison-feux", "10", function(d) {
  mois_labs <- c("Jan", "Fév", "Mar", "Avr", "Mai", "Jun", "Jul", "Aoû", "Sep", "Oct", "Nov", "Déc")
...
```{r fig_evol_feux}
#| label: fig-evol-feux
#| fig-cap: "Évolution perçue des feux depuis 3 ans, par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| fig-cap: "Les feux sont-ils un problème dans la région ? (par observatoire). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_reglementation_feux}
#| label: fig-reglementation-feux
#| fig-cap: "La pratique du feu devrait être… (par observatoire). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig-connaissance-ap}
#| label: fig-connaissance-ap
#| fig-cap: "Connaissance de l'Aire protégée et demande de protection (% de Oui). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| echo: false
#| results: asis
dl_fig_variants("fig-connaissance-ap", "10", function(d) {
  pct_v <- function(df, var, label) {
    var_sym <- sym(var)
    df |> summarise(pct = round(sum(!!var_sym == 1, na.rm = TRUE) / sum(!is.na(!!var_sym)) * 100, 0), .by = Observatory) |> mutate(Item = label)
  }
...
#| fig-cap: "Jugement des actions de l'AP sur la situation économique, sociale et environnementale. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
#| fig-width: 12
#| fig-height: 7
...
ggplot(ap3_data, aes(x = Dimension, y = pct, fill = Jugement)) +
  ...
  coord_flip() +
  facet_wrap(~Observatory, ncol = 1) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_fill_manual(
    values = c(
      "Très positif" = "#2ca02c",
      "Plutôt positif" = "#98df8a",
      "Plutôt négatif" = "#ff9896",
      "Très négatif" = "#d62728",
...
dl_fig_variants("fig_jugement_ap", "10", function(d) {
  ...
  ap3_data <- d |>
    ...
    mutate(
      ...
      Dimension = factor(Dimension, levels = c("Env.", "Éco.", "Sociale")),
      Jugement = factor(ap3_labels[as.character(Valeur)], levels = ap3_labels)
    ) |>
    filter(!is.na(Jugement)) |>
    count(Observatory, Dimension, Jugement) |>
...
#| fig-cap: "Qui devrait gérer l'AP? (parmi ceux favorables à la protection) .<br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| echo: false
#| results: asis
dl_fig_variants("fig-gestionnaire", "10", function(d) {
...
```{r fig_mesures_protection}
#| label: fig-mesures-protection
#| fig-cap: "Mesures souhaitées pour protéger l'AP (% de Oui, parmi ceux favorables). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
```{r fig_priorite}
#| label: fig-priorite
#| fig-cap: "Pauvreté vs. protection de l'environnement : quelle priorité ? (par observatoire). <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."
...
#| fig-cap: "Information, consultation et bénéfice des actions de l'AP (% de Oui), par observatoire. <br>Source : Enquêtes auprès des OR Alaotra et Marovoay - 2025, projet BETSAKA, CERED, IRD."

...
ggplot(qual, aes(x = Qualité, y = n, fill = Observatory)) +
  ...
  labs(x = NULL, y = "Nombre d'entretiens") +
...
ggplot(qual_rep, aes(x = Qualité, y = pct, fill = Observatory)) +
  geom_col(show.legend = FALSE) +
...
ggplot(reaction, aes(x = Réaction, y = pct, fill = Observatory)) +
  geom_col(show.legend = FALSE) +
  geom_text(aes(label = paste0(pct, "%")), vjust = -0.3, size = 3) +
  labs(x = NULL, y = "%") +
  facet_wrap(~Observatory) +
